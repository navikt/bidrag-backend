...
import no.nav.bidrag.beregn.barnebidrag.service.orkestrering.SkalAldersjusteresManueltBegrunnelse
...
class AldersjusteringOrchestratorTest {
    ...
    fun `skal beregne aldersjustering`() {
        ...
        assertSoftly(beregning) {
            ...
            it.beregnetBarnebidragPeriodeListe.first().resultat.beløp shouldBe BigDecimal(5360)
            it.beregnetBarnebidragPeriodeListe.first().periode.fom shouldBe YearMonth.now().withMonth(7)
    ...
    fun `skal kaste skal ikke aldersjusteringsfeil hvis siste manuelle vedtak resultat var redusert av evne`() {
        ...
        val exception = shouldThrow<SkalIkkeAldersjusteresException> {
            aldersjusteringOrchestrator.utførAldersjustering(
                ...
                aldersjusteresForÅr = LocalDate.now().year,
            )
        }
    ...
    fun `skal kaste skal aldersjusteres manielt hvis mangler grunnlag`() {
        ...
        exception.message shouldBe "Aldersjustering kunne ikke utføres fordi vedtak -1 mangler grunnlag"
    }

    @Test
    fun `skal kaste skal aldersjusteres manuelt hvis siste vedtak har ingen perioder i stønadsendringen`() {
        val vedtak = konverterVedtakFraFil()
        every { beregningVedtakConsumer.hentVedtak(eq(vedtaksidBidrag)) } returns vedtak.copy(
        ...
        exception.begrunnelse shouldBe SkalAldersjusteresManueltBegrunnelse.FANT_INGEN_MANUELL_VEDTAK
    }
    ...
    // --- Tests for finnSisteManuelleVedtakIgnorerFremtidigOpphør boundary conditions ---

    @Test
    fun `finnSisteManuelleVedtakIgnorerFremtidigOpphør - skal returnere forrige vedtak når opphør fom er etter aldersjusteringPeriodeFom`() {
        val aldersjusteringPeriodeFom = YearMonth.now().withMonth(7)
        val opphørFom = aldersjusteringPeriodeFom.plusMonths(1) // August - fremtidig opphør

        settOppOpphørVedtakMock(opphørFom)

        val resultat = aldersjusteringOrchestrator.finnSisteManuelleVedtakIgnorerFremtidigOpphør(bidragStønadsid, aldersjusteringPeriodeFom)

        resultat.vedtaksId shouldBe vedtaksidBidrag
    }

    @Test
    fun `finnSisteManuelleVedtakIgnorerFremtidigOpphør - skal returnere opphørvedtak når opphør fom er lik aldersjusteringPeriodeFom`() {
        val aldersjusteringPeriodeFom = YearMonth.now().withMonth(7)
        val opphørFom = aldersjusteringPeriodeFom // Samme måned - skal IKKE ignoreres

        settOppOpphørVedtakMock(opphørFom)

        val resultat = aldersjusteringOrchestrator.finnSisteManuelleVedtakIgnorerFremtidigOpphør(bidragStønadsid, aldersjusteringPeriodeFom)

        resultat.vedtaksId shouldBe opphørVedtaksid
    }

    @Test
    fun `finnSisteManuelleVedtakIgnorerFremtidigOpphør - skal returnere opphørvedtak når opphør fom er før aldersjusteringPeriodeFom`() {
        val aldersjusteringPeriodeFom = YearMonth.now().withMonth(7)
        val opphørFom = aldersjusteringPeriodeFom.minusMonths(1) // Juni - opphør før aldersjustering
    ...
    @Test
    fun `skal beregne aldersjustering når siste vedtak er fremtidig opphør etter aldersjusteringPeriodeFom`() {
        val fødselsdatoBarn = LocalDate.now().minusYears(11)
        every { personConsumer.hentFødselsdatoForPerson(eq(Personident(personIdentSøknadsbarn1))) } returns fødselsdatoBarn
        val aldersjusteringPeriodeFom = YearMonth.now().withMonth(7)
        val opphørFom = aldersjusteringPeriodeFom.plusMonths(1) // Fremtidig opphør - skal ignoreres

        settOppOpphørVedtakMock(opphørFom)

        val (_, _, beregning) = aldersjusteringOrchestrator.utførAldersjustering(
            stønad = bidragStønadsid,
        )

        beregning.shouldNotBeNull()
        beregning.beregnetBarnebidragPeriodeListe.shouldHaveSize(1)
    }

    @Test
    fun `skal kaste BIDRAGET_HAR_OPPHØRT når opphør fom er lik aldersjusteringPeriodeFom`() {
        val aldersjusteringPeriodeFom = YearMonth.now().withMonth(7)
        val opphørFom = aldersjusteringPeriodeFom // Opphør i juli - skal IKKE ignoreres
    ...
    @Test
    fun `skal kaste BIDRAGET_HAR_OPPHØRT når opphør fom er før aldersjusteringPeriodeFom`() {
        val aldersjusteringPeriodeFom = YearMonth.now().withMonth(7)
        val opphørFom = aldersjusteringPeriodeFom.minusMonths(1) // Opphør i juni - skal IKKE ignoreres
    ...
    // --- End of finnSisteManuelleVedtakIgnorerFremtidigOpphør tests ---

    @Test
    fun `skal beregne aldersjustering selv om beregnet beløp er under løpende bidrag`() {
        val fødselsdatoBarn = LocalDate.now().minusYears(11)
        every { personConsumer.hentFødselsdatoForPerson(eq(Personident(personIdentSøknadsbarn1))) } returns fødselsdatoBarn
        every { beregningBeløpshistorikkConsumer.hentHistoriskeStønader(any()) } returns
            opprettStønadDto(
        ...
        shouldNotThrow<SkalIkkeAldersjusteresException> {
            aldersjusteringOrchestrator.utførAldersjustering(
                stønad = Stønadsid(
                    type = Stønadstype.BIDRAG,
                    kravhaver = Personident(personIdentSøknadsbarn1),
...
private val opphørVedtaksid = 100

private val bidragStønadsid = Stønadsid(
    type = Stønadstype.BIDRAG,
    kravhaver = Personident(personIdentSøknadsbarn1),
    skyldner = Personident(personIdentBidragspliktig),
    sak = Saksnummer(saksnummer),
...
fun AldersjusteringOrchestratorTest.settOppOpphørVedtakMock(opphørFom: YearMonth) {
    val opphørVedtakDto = byggOpphørVedtakDto(opphørFom)
    every { beregningVedtakConsumer.hentVedtakForStønad(any()) } returns
        HentVedtakForStønadResponse(
            vedtakListe = listOf(
                opprettVedtakForStønad(personIdentSøknadsbarn1, stønadstype = Stønadstype.BIDRAG).copy(
                    vedtaksid = opphørVedtaksid,
                    type = Vedtakstype.ENDRING,
                    vedtakstidspunkt = LocalDateTime.parse("2025-06-01T00:00:00"),
                ),
                opprettVedtakForStønad(personIdentBidragspliktig, stønadstype = Stønadstype.FORSKUDD).copy(
                    vedtaksid = vedtaksidBidrag,
                    type = Vedtakstype.ENDRING,
    ...
    every { beregningVedtakConsumer.hentVedtak(eq(opphørVedtaksid)) } returns opphørVedtakDto
}

fun byggOpphørVedtakDto(opphørFom: YearMonth): VedtakDto {
    val vedtak = konverterVedtakFraFil()
    return vedtak.copy(
        stønadsendringListe = listOf(
            vedtak.stønadsendringListe.first { it.kravhaver.verdi == personIdentSøknadsbarn1 }.copy(
                periodeListe = listOf(
                    VedtakPeriodeDto(
                        beløp = null,
                        delytelseId = "1",
                        periode = ÅrMånedsperiode(opphørFom, null),
                        resultatkode = Resultatkode.OPPHØR.name,
                        valutakode = null,
                        grunnlagReferanseListe = emptyList(),
                    ),
...
fun konverterVedtakFraFil(filnavn: String = "vedtak_aldersjustering_1.json"): VedtakDto {
    var json = hentFil("/testfiler/aldersjustering_orkestrator/$filnavn").readText()
    json = json.replace("{FNR_HM1}", genererFødselsnummer())
    json = json.replace("{FNR_HM2}", genererFødselsnummer())
    return commonObjectmapper.readValue(json)
}
