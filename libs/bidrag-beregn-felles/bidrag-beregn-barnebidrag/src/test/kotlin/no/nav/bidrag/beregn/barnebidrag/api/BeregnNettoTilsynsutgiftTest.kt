...
internal class BeregnNettoTilsynsutgiftTest : FellesTest() {
    ...
    fun testBeregnMånedsbeløpFaktiskUtgiftTilleggsstønad() {
        ...
        val responseTilleggsstønad = api.beregnMånedsbeløpTilleggsstønad(tilleggsstønad, InntektBeløpstype.DAGSATS)

        assertThat(responseFaktiskUtgift).isEqualByComparingTo(BigDecimal.valueOf(550))
        assertThat(responseTilleggsstønad).isEqualByComparingTo(BigDecimal.valueOf(337.64))
    ...
    @Test
    fun testFiltreringSjablonerMotBruddperiode() {
        val bruddperiode = ÅrMånedsperiode(YearMonth.of(2025, 9), YearMonth.of(2026, 1))
        val grunnlagListe = listOf(
            SjablonSjablontallPeriodeGrunnlag(
                referanse = "ref1",
                sjablonSjablontallPeriode = SjablonSjablontallPeriode(
                    periode = ÅrMånedsperiode(YearMonth.of(2023, 1), YearMonth.of(2026, 1)),
                    sjablon = SjablonTallNavn.SKATT_ALMINNELIG_INNTEKT_PROSENT,
                    verdi = BigDecimal.valueOf(24.85),
                ),
            ),
            SjablonSjablontallPeriodeGrunnlag(
                referanse = "ref2",
                sjablonSjablontallPeriode = SjablonSjablontallPeriode(
                    periode = ÅrMånedsperiode(YearMonth.of(2026, 1), null),
                    sjablon = SjablonTallNavn.SKATT_ALMINNELIG_INNTEKT_PROSENT,
                    verdi = BigDecimal.valueOf(24.85),
                ),
            ),
        ...
        assertEquals("ref1", result[0].referanse)
        assertEquals(SjablonTallNavn.SKATT_ALMINNELIG_INNTEKT_PROSENT.navn, result[0].type)
    }

    @Test
    @DisplayName("Netto tilsynsutgift - eksempel med månedsbeløp tilleggsstønad")
    fun testNettoTilsynsutgift_MånedsbeløpTilleggsstønad() {
        filnavn = "src/test/resources/testfiler/nettotilsynsutgift/nettotilsynsutgift_eksempel_månedsbeløp_tilleggsstønad.json"
        val resultat = utførBeregningerOgEvaluerResultatNettoTilsynsutgift()
        ...
        assertSoftly {
            ...
            // Resultat periode 0
            resultat[0].periode shouldBe ÅrMånedsperiode("2024-01", "2024-02")
            resultat[0].totalTilsynsutgift shouldBe BigDecimal.valueOf(366.67)
            resultat[0].justertBruttoTilsynsutgift shouldBe BigDecimal.valueOf(366.67)
            resultat[0].andelTilsynsutgiftFaktor.stripTrailingZeros() shouldBe BigDecimal.valueOf(1)
            resultat[0].skattefradrag shouldBe BigDecimal.valueOf(91.12)
            resultat[0].nettoTilsynsutgift shouldBe BigDecimal.valueOf(275.55)
            resultat[0].tilsynsutgiftBarnListe[0].sumTilsynsutgifter shouldBe BigDecimal.valueOf(366.67)
            resultat[0].tilsynsutgiftBarnListe[0].endeligSumTilsynsutgifter shouldBe BigDecimal.valueOf(366.67)

            // Resultat periode 1
            resultat[1].periode shouldBe ÅrMånedsperiode("2024-02", "2024-07")
            resultat[1].totalTilsynsutgift.stripTrailingZeros() shouldBe BigDecimal.valueOf(275)
            resultat[1].justertBruttoTilsynsutgift.stripTrailingZeros() shouldBe BigDecimal.valueOf(275)
            resultat[1].andelTilsynsutgiftFaktor.stripTrailingZeros() shouldBe BigDecimal.valueOf(1)
            resultat[1].skattefradrag shouldBe BigDecimal.valueOf(68.34)
            resultat[1].nettoTilsynsutgift shouldBe BigDecimal.valueOf(206.66)
            resultat[1].tilsynsutgiftBarnListe[0].sumTilsynsutgifter.stripTrailingZeros() shouldBe BigDecimal.valueOf(275)
            resultat[1].tilsynsutgiftBarnListe[0].endeligSumTilsynsutgifter.stripTrailingZeros() shouldBe BigDecimal.valueOf(275)

            // Resultat periode 2
            resultat[2].periode shouldBe ÅrMånedsperiode(YearMonth.parse("2024-07"), null)
            resultat[2].totalTilsynsutgift.stripTrailingZeros() shouldBe BigDecimal.valueOf(275)
            resultat[2].justertBruttoTilsynsutgift.stripTrailingZeros() shouldBe BigDecimal.valueOf(275)
            resultat[2].andelTilsynsutgiftFaktor.stripTrailingZeros() shouldBe BigDecimal.valueOf(1)
            resultat[2].skattefradrag shouldBe BigDecimal.valueOf(68.34)
            resultat[2].nettoTilsynsutgift shouldBe BigDecimal.valueOf(206.66)
            resultat[2].tilsynsutgiftBarnListe[0].sumTilsynsutgifter.stripTrailingZeros() shouldBe BigDecimal.valueOf(275)
            resultat[2].tilsynsutgiftBarnListe[0].endeligSumTilsynsutgifter.stripTrailingZeros() shouldBe BigDecimal.valueOf(275)
        }
    }

    @Test
    @DisplayName("Netto tilsynsutgift - kast exception hvis hverken dag eller månedssats er angitt for tilleggsstønad")
    fun testNettoTilsynsutgift_hverken_dagsats_eller_månedssats_i_input_tilleggsstønad_skal_kaste_exception() {
        filnavn = "src/test/resources/testfiler/nettotilsynsutgift/nettotilsynsutgift_eksempel_uten_beløp_tilleggsstønad.json"

        val request = lesFilOgByggRequest(filnavn)

        val exception = assertThrows(IllegalArgumentException::class.java) {
            api.beregnNettoTilsynsutgift(request)
        }
        assertThat(exception.message).isEqualTo(
            "Ugyldig input ved beregning av netto tilsynsutgifter. Ugyldig beløpstype angitt for tilleggsstønad med referanse Mottatt_Tilleggsstønad_202301_SB1",
        )
    }
}
