...
internal object NettoTilsynsutgiftMapper : CoreMapper() {
        ...
        try {
            val barnBMListe =
                beregnGrunnlag.grunnlagListe
                    ...
                    .filter { it.innhold.bidragsmottaker == beregnGrunnlag.grunnlagListe.bidragsmottaker?.referanse }
                    .map {
                        BarnBM(
                    ...
                    beregnGrunnlag.grunnlagListe
                        ...
                        .filter { it.gjelderReferanse == beregnGrunnlag.grunnlagListe.bidragsmottaker?.referanse }
                        .map {
                            BarnBM(
                                referanse = it.referanse,
                                fødselsdato = it.innhold.fødselsdato,
        ...
        try {
            val tilleggsstønadListe =
                beregnGrunnlag.grunnlagListe
                    ...
                    .map {
                        val beregnetBeløp = when (it.innhold.beløpstype) {
                            InntektBeløpstype.DAGSATS -> beregnMånedsbeløpTilleggsstønad(it.innhold.beløp, it.innhold.beløpstype)

                            InntektBeløpstype.MÅNEDSBELØP_11_MÅNEDER -> beregnMånedsbeløpTilleggsstønad(it.innhold.beløp, it.innhold.beløpstype)

                            else -> throw IllegalArgumentException(
                                "Ugyldig beløpstype angitt for tilleggsstønad med referanse ${it.referanse}",
                            )
                        }
            ...
            throw IllegalArgumentException(
                "Ugyldig input ved beregning av netto tilsynsutgifter. " + e.message,
            )
        }
    }
    ...
    fun beregnMånedsbeløpTilleggsstønad(beløp: BigDecimal, beløpstype: InntektBeløpstype): BigDecimal = when (beløpstype) {
        ...
        InntektBeløpstype.MÅNEDSBELØP_11_MÅNEDER -> beløp.multiply(BigDecimal.valueOf(11))
            .divide(BigDecimal.valueOf(12), 10, RoundingMode.HALF_UP).coerceAtLeast(BigDecimal.ZERO) ?: BigDecimal.ZERO
