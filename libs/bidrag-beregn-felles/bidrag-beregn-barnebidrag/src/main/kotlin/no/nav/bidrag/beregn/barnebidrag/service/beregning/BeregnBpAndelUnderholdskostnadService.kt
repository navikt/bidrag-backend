...
internal object BeregnBpAndelUnderholdskostnadService : BeregnService() {
    ...
    fun delberegningBpAndelUnderholdskostnad(
        ...
        return slåSammenLikeObjekter(
            ...
            grunnlagstype = Grunnlagstype.DELBEREGNING_BIDRAGSPLIKTIGES_ANDEL,
            utvidPeriodeTil = ::utvidBidragspliktigesAndelPeriodeTil,
        )
    }

    // Spesifikk implementasjon for å utvide periode for DelberegningBidragspliktigesAndel. Slår sammen fra-periode fra forrige objekt og til-periode
    // fra nåværende objekt og beholder resten av innholdet fra forrige objekt.
    private fun utvidBidragspliktigesAndelPeriodeTil(forrigeGrunnlag: GrunnlagDto, nåværendeGrunnlag: GrunnlagDto): GrunnlagDto {
        val forrigeBidragspliktigesAndel = forrigeGrunnlag.tilInnholdMedReferanse<DelberegningBidragspliktigesAndel>()
        val nåværendeBidragspliktigesAndel = nåværendeGrunnlag.tilInnholdMedReferanse<DelberegningBidragspliktigesAndel>()

        val utvidetPeriode = forrigeBidragspliktigesAndel.innhold.periode.copy(
            til = nåværendeBidragspliktigesAndel.innhold.periode.til,
        )
        val utvidetInnhold = forrigeBidragspliktigesAndel.innhold.copy(periode = utvidetPeriode)
