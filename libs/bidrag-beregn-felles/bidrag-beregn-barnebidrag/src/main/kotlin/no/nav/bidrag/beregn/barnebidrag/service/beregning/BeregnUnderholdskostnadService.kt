...
internal object BeregnUnderholdskostnadService : BeregnService() {
    ...
    fun delberegningUnderholdskostnad(
        ...
        bruddPeriodeListe.forEach { bruddPeriode ->
            ...
            val barnetrygdType = when {
                ...
                bruddPeriode.fom.compareTo(søknadsbarnFødselsdatoÅrMåned) == 0 -> BarnetrygdType.INGEN
        ...
        return slåSammenLikeObjekter(
            ...
            grunnlagstype = Grunnlagstype.DELBEREGNING_UNDERHOLDSKOSTNAD,
            utvidPeriodeTil = ::utvidUnderholdskostnadPeriodeTil,
        )
    }

    // Spesifikk implementasjon for å utvide periode for DelberegningUnderholdskostnad. Slår sammen fra-periode fra forrige objekt og til-periode fra
    // nåværende objekt og beholder resten av innholdet fra forrige objekt.
    private fun utvidUnderholdskostnadPeriodeTil(forrigeGrunnlag: GrunnlagDto, nåværendeGrunnlag: GrunnlagDto): GrunnlagDto {
        val forrigeUnderholdskostnad = forrigeGrunnlag.tilInnholdMedReferanse<DelberegningUnderholdskostnad>()
        val nåværendeUnderholdskostnad = nåværendeGrunnlag.tilInnholdMedReferanse<DelberegningUnderholdskostnad>()

        val utvidetPeriode = forrigeUnderholdskostnad.innhold.periode.copy(
            til = nåværendeUnderholdskostnad.innhold.periode.til,
        )
        val utvidetInnhold = forrigeUnderholdskostnad.innhold.copy(periode = utvidetPeriode)
