...
fun List<GrunnlagDto>.finnPersonGrunnlag(ident: String?, fødselsdato: LocalDate): BaseGrunnlag? = (
    // Kan hende ident er endret siden siste vedtak. Hent derfor basert på fødselsdato som fallback
    ident?.let { identverdi ->
        hentAllePersoner().find {
            it.personIdent == identverdi
        }
    } ?: hentAllePersoner().singleOrNull { it.personObjekt.fødselsdato == fødselsdato }
    )
