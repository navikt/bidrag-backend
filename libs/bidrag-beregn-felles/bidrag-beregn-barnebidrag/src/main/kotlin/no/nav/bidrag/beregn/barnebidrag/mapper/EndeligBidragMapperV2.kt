...
import no.nav.bidrag.beregn.barnebidrag.bo.BidragTilFordelingPrivatAvtaleDelberegningPeriodeGrunnlag
...
import no.nav.bidrag.beregn.barnebidrag.bo.IndeksreguleringPrivatAvtaleDelberegningPeriodeGrunnlag
...
import no.nav.bidrag.beregn.barnebidrag.bo.PrivatAvtaleGrunnlagTilBeregning
...
internal object EndeligBidragMapperV2 : CoreMapper() {
    ...
    fun mapBidragTilFordelingGrunnlag(mottattGrunnlag: BeregnGrunnlag) = BidragTilFordelingPeriodeGrunnlag(
        ...
        bidragspliktigesAndelDeltBostedDelberegningPeriodeGrunnlagListe = mapBidragspliktigesAndelDeltBosted(
            beregnGrunnlag = mottattGrunnlag.grunnlagListe,
            gjelderBarnReferanse = mottattGrunnlag.søknadsbarnReferanse,
        ),
    ...
    fun mapAndelAvBidragsevneGrunnlag(mottattGrunnlag: BeregnGrunnlag) = AndelAvBidragsevnePeriodeGrunnlag(
        ...
        bidragTilFordelingDelberegningPeriodeGrunnlagListe = mapBidragTilFordeling(
            grunnlagListe = mottattGrunnlag.grunnlagListe,
            gjelderBarnReferanse = mottattGrunnlag.søknadsbarnReferanse,
        ),
    )
    ...
    fun mapSluttberegningBarnebidragGrunnlag(mottattGrunnlag: BeregnGrunnlag) = SluttberegningBarnebidragV2PeriodeGrunnlag(
        ...
        bpAndelUnderholdskostnadDelberegningPeriodeGrunnlagListe = mapBpAndelUnderholdskostnad(mottattGrunnlag),
        bostatusPeriodeGrunnlagListe = mapBostatus(beregnGrunnlag = mottattGrunnlag, søknadsbarnReferanse = mottattGrunnlag.søknadsbarnReferanse),
        bidragspliktigesAndelDeltBostedDelberegningPeriodeGrunnlagListe = mapBidragspliktigesAndelDeltBosted(
            beregnGrunnlag = mottattGrunnlag.grunnlagListe,
            gjelderBarnReferanse = mottattGrunnlag.søknadsbarnReferanse,
        ),
    ...
    private fun mapBpAndelUnderholdskostnad(beregnGrunnlag: BeregnGrunnlag): List<BpAndelUnderholdskostnadDelberegningPeriodeGrunnlag> {
        try {
            return beregnGrunnlag.grunnlagListe
                .filtrerOgKonverterBasertPåFremmedReferanse<DelberegningBidragspliktigesAndel>(
                    grunnlagType = Grunnlagstype.DELBEREGNING_BIDRAGSPLIKTIGES_ANDEL,
                    gjelderBarnReferanse = beregnGrunnlag.søknadsbarnReferanse,
                )
                .map {
    ...
    private fun mapSamværsfradrag(beregnGrunnlag: List<GrunnlagDto>, gjelderBarnReferanse: String): List<SamværsfradragDelberegningPeriodeGrunnlag> {
        try {
            return beregnGrunnlag
                .filtrerOgKonverterBasertPåFremmedReferanse<DelberegningSamværsfradrag>(
                    grunnlagType = Grunnlagstype.DELBEREGNING_SAMVÆRSFRADRAG,
    ...
    private fun mapBidragspliktigesAndelDeltBosted(
        ...
        try {
            return beregnGrunnlag
                .filtrerOgKonverterBasertPåFremmedReferanse<DelberegningBidragspliktigesAndelDeltBosted>(
                    grunnlagType = Grunnlagstype.DELBEREGNING_BIDRAGSPLIKTIGES_ANDEL_DELT_BOSTED,
                    gjelderBarnReferanse = gjelderBarnReferanse,
                )
                .map {
    ...
    ): List<BidragTilFordelingDelberegningPeriodeGrunnlag> = mapBidragTilFordeling(grunnlagListe = beregnGrunnlagListe.flatMap { it.grunnlagListe })

    private fun mapBidragTilFordeling(
        grunnlagListe: List<GrunnlagDto>,
        gjelderBarnReferanse: String? = null,
    ): List<BidragTilFordelingDelberegningPeriodeGrunnlag> {
        try {
            return grunnlagListe
                .filtrerOgKonverterBasertPåFremmedReferanse<DelberegningBidragTilFordeling>(
                    grunnlagType = Grunnlagstype.DELBEREGNING_BIDRAG_TIL_FORDELING,
                    gjelderBarnReferanse = gjelderBarnReferanse,
                )
                .map {
    ...
    private fun mapIndeksreguleringPrivatAvtale(beregnGrunnlag: BeregnGrunnlag): List<IndeksreguleringPrivatAvtaleDelberegningPeriodeGrunnlag> {
        try {
            return beregnGrunnlag.grunnlagListe
                ...
                .map {
                    IndeksreguleringPrivatAvtaleDelberegningPeriodeGrunnlag(
                        referanse = it.referanse,
                        privatAvtalePeriode = it.innhold,
                    )
                }
        } catch (e: Exception) {
            throw IllegalArgumentException(
                "Ugyldig input ved beregning av barnebidrag. Innhold i Grunnlagstype.DELBEREGNING_INDEKSREGULERING_PRIVAT_AVTALE er ikke gyldig: " +
                    e.message,
            )
        }
    }

    private fun mapBidragTilFordelingPrivatAvtale(
        grunnlagListe: List<GrunnlagDto>,
    ): List<BidragTilFordelingPrivatAvtaleDelberegningPeriodeGrunnlag> {
        try {
            return grunnlagListe
                ...
                .map {
                    BidragTilFordelingPrivatAvtaleDelberegningPeriodeGrunnlag(
                        referanse = it.referanse,
                        bidragTilFordelingPrivatAvtalePeriode = it.innhold,
                    )
                }
        } catch (e: Exception) {
            throw IllegalArgumentException(
                "Ugyldig input ved beregning av barnebidrag. Innhold i Grunnlagstype.DELBEREGNING_BIDRAG_TIL_FORDELING_PRIVAT_AVTALE er ikke " +
                    "gyldig: " + e.message,
            )
        }
    }
    ...
    private fun mapBidragsevne(beregnGrunnlag: BeregnGrunnlag): List<BidragsevneDelberegningPeriodeGrunnlag> {
        try {
            return beregnGrunnlag.grunnlagListe
                .filtrerOgKonverterBasertPåFremmedReferanse<DelberegningBidragsevne>(
                    grunnlagType = Grunnlagstype.DELBEREGNING_BIDRAGSEVNE,
                    gjelderBarnReferanse = beregnGrunnlag.søknadsbarnReferanse,
                )
                .map {
    ...
    private fun mapEvne25ProsentAvInntekt(beregnGrunnlag: BeregnGrunnlag): List<Evne25ProsentAvInntektDelberegningPeriodeGrunnlag> {
        try {
            return beregnGrunnlag.grunnlagListe
                .filtrerOgKonverterBasertPåFremmedReferanse<DelberegningEvne25ProsentAvInntekt>(
                    grunnlagType = Grunnlagstype.DELBEREGNING_EVNE_25PROSENTAVINNTEKT,
                    gjelderBarnReferanse = beregnGrunnlag.søknadsbarnReferanse,
                )
                .map {
        ...
        try {
            return beregnGrunnlag.grunnlagListe
                .filtrerOgKonverterBasertPåFremmedReferanse<DelberegningAndelAvBidragsevne>(
                    grunnlagType = Grunnlagstype.DELBEREGNING_ANDEL_AV_BIDRAGSEVNE,
                    gjelderBarnReferanse = beregnGrunnlag.søknadsbarnReferanse,
                )
                .map {
    ...
    private fun mapBidragJustertForBPBarnetillegg(beregnGrunnlag: BeregnGrunnlag): List<BidragJustertForBPBarnetilleggDelberegningPeriodeGrunnlag> {
        try {
            return beregnGrunnlag.grunnlagListe
                .filtrerOgKonverterBasertPåFremmedReferanse<DelberegningBidragJustertForBPBarnetillegg>(
                    grunnlagType = Grunnlagstype.DELBEREGNING_BIDRAG_JUSTERT_FOR_BP_BARNETILLEGG,
                    gjelderBarnReferanse = beregnGrunnlag.søknadsbarnReferanse,
                )
                .map {
    ...
    private fun mapValutakurs(grunnlag: GrunnlagDto?): ValutakursGrunnlagTilBeregning? {
        try {
            val valutakursGrunnlag: ValutakursGrunnlag? = grunnlag?.innholdTilObjekt<ValutakursGrunnlag>()
            return valutakursGrunnlag?.let {
                ValutakursGrunnlagTilBeregning(
                    referanse = grunnlag.referanse,
                    valutakursListe = it.valutakursListe,
                )
            }
        } catch (e: Exception) {
            throw IllegalArgumentException(
                "Ugyldig input ved beregning av barnebidrag. Innhold i Grunnlagstype.VALUTAKURS_GRUNNLAG er ikke gyldig: " + e.message,
            )
        }
    }

    private fun mapPrivatAvtaleGrunnlag(beregnGrunnlag: BeregnGrunnlag): PrivatAvtaleGrunnlagTilBeregning {
        try {
            val privatAvtale = beregnGrunnlag.grunnlagListe
                .filtrerOgKonverterBasertPåEgenReferanse<PrivatAvtaleGrunnlagV2>(Grunnlagstype.PRIVAT_AVTALE_GRUNNLAG)
                .firstOrNull()
                ?: throw IllegalArgumentException("Fant ingen privat avtale grunnlag.")

            return PrivatAvtaleGrunnlagTilBeregning(
                referanse = privatAvtale.referanse,
                sakskategori = privatAvtale.innhold.sakskategori,
            )
        } catch (e: Exception) {
            throw IllegalArgumentException(
                "Ugyldig input ved beregning av barnebidrag. Innhold i Grunnlagstype.PRIVAT_AVTALE_GRUNNLAG er ikke gyldig: " +
                    e.message,
            )
        }
    }
