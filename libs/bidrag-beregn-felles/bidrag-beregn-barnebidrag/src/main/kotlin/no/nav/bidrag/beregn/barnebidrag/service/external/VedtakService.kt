...
class VedtakService(
    ...
    private fun List<VedtakForStønad>.hentPåklagetVedtakListe(vedtak: VedtakForStønad): Set<PåklagetVedtak> {
        ...
        if (refererTilVedtakId.isNotEmpty()) {
            return refererTilVedtakId
                .flatMap { vedtaksid ->
                    val opprinneligVedtak = find { it.vedtaksid == vedtaksid } ?: return@flatMap emptyList()
    ...
    fun finnSisteManuelleVedtak(stønadsid: Stønadsid, ignorerVedtaksid: List<Int>? = null): SisteManuelleVedtak? {
        val vedtakslisteForStønad =
            vedtakConsumer.hentVedtakForStønad(
                HentVedtakForStønadRequest(
                    stønadsid.sak,
                    stønadsid.type,
            ...
            ).vedtakListe.filter { it.vedtaksid !in (ignorerVedtaksid ?: emptyList()) }
        val resultat =
            vedtakFilter.finneSisteManuelleVedtak(vedtakslisteForStønad) ?: return null
    ...
    fun hentOpprinneligVedtakHvisOrkestrertVedtak(sisteManuelleVedtak: SisteManuelleVedtak): SisteManuelleVedtak {
        val vedtak = sisteManuelleVedtak.vedtak
        val referertVedtak =
            if (vedtak.harResultatFraAnnenVedtak && (vedtak.erOrkestrertVedtak || vedtak.erDelvedtak)) {
                vedtakConsumer.hentVedtak(vedtak.referertVedtaksid!!)?.let {
                    hentOpprinneligVedtakHvisOrkestrertVedtak(SisteManuelleVedtak(it.vedtaksid, it)).vedtak
                } ?: vedtak
            } else {
                vedtak
            }
        return SisteManuelleVedtak(referertVedtak.vedtaksid, referertVedtak)
    }
    fun finnAlleManuelleVedtakForBp(request: HentManuelleVedtakRequest): List<VedtakForStønad> {
        val manuelleVedtakForBp =
            vedtakConsumer.hentManuelleVedtak(
                HentManuelleVedtakRequest(
                    skyldner = request.skyldner,
                ),
            )
        // kjører denne for å håndtere klager riktig, usikker på om det er nødvendig her
        // Filtrering for manuelle vedtak gjøres allerede i bidrag-vedtak, men beholder logikken for å sikre riktig behandling ved klage.
        return vedtakFilter.finneAlleManuelleVedtak(manuelleVedtakForBp.vedtakListe)
    }
