...
internal object BeregnNettoTilsynsutgiftService : BeregnService() {
    ...
    fun delberegningNettoTilsynsutgift(
        ...
        val nettoTilsynsutgiftPeriodeGrunnlag =
            NettoTilsynsutgiftMapper.mapNettoTilsynsutgiftPeriodeGrunnlag(mottattGrunnlag = mottattGrunnlag, sjablonGrunnlag = sjablonGrunnlag)

        val bruddPeriodeListe = lagBruddPeriodeListeNettoTilsynsutgift(
            ...
            if (barnMedUtgifterIPerioden.isNotEmpty()) {
                val nettoTilsynsutgiftBeregningGrunnlag =
                    lagNettoTilsynsutgiftBeregningGrunnlag(
                        ...
                        barnMedUtgifterIPerioden = barnMedUtgifterIPerioden,
                    )
        ...
        return slåSammenLikeObjekter(
            ...
            grunnlagstype = Grunnlagstype.DELBEREGNING_NETTO_TILSYNSUTGIFT,
            utvidPeriodeTil = ::utvidNettoTilsynsutgiftPeriodeTil,
        )
    }

    // Spesifikk implementasjon for å utvide periode for DelberegningNettoTilsynsutgift. Slår sammen fra-periode fra forrige objekt og til-periode fra
    // nåværende objekt og beholder resten av innholdet fra forrige objekt.
    private fun utvidNettoTilsynsutgiftPeriodeTil(forrigeGrunnlag: GrunnlagDto, nåværendeGrunnlag: GrunnlagDto): GrunnlagDto {
        val forrigeNettoTilsynsutgift = forrigeGrunnlag.tilInnholdMedReferanse<DelberegningNettoTilsynsutgift>()
        val nåværendeNettoTilsynsutgift = nåværendeGrunnlag.tilInnholdMedReferanse<DelberegningNettoTilsynsutgift>()

        val utvidetPeriode = forrigeNettoTilsynsutgift.innhold.periode.copy(
            til = nåværendeNettoTilsynsutgift.innhold.periode.til,
        )
        val utvidetInnhold = forrigeNettoTilsynsutgift.innhold.copy(periode = utvidetPeriode)
    ...
    private fun lagBruddPeriodeListeNettoTilsynsutgift(
        ...
        val fødselsmånederForBarnFødtEtterberegningsperiodeFra = grunnlagListe.barnBMListe
            ...
            .map { ÅrMånedsperiode(fom = YearMonth.from(it.fødselsdato), til = YearMonth.from(it.fødselsdato)) }
            .distinct()
        ...
        return lagBruddPeriodeListe(periodeListe = periodeListe, beregningsperiode = beregningsperiode)
    }
