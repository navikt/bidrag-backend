...
import no.nav.bidrag.beregn.barnebidrag.service.beregning.BeregnBarnebidragService.Companion.lagResultatPerioderV2
import no.nav.bidrag.beregn.barnebidrag.service.beregning.BeregnEndringSjekkGrensePeriodeService.delberegningEndringSjekkGrensePeriodeV2
...
val List<ResultatVedtak>.omgjøringsvedtaksErEnesteVedtak
    get() =
...
internal data class BeløpshistorikkPeriodeInternal(
    ...
    val revurderAldersjustering: Boolean = false,
...
@Import(
    ...
    IdentUtils::class,
    OmgjøringOrkestratorHelpersV2::class,
)
class OmgjøringOrkestratorV2(
    ...
    private val omgjøringOrkestratorHelpers: OmgjøringOrkestratorHelpersV2,
) {

    fun utførOmgjøringEndelig(
        omgjøringResultat: BeregnetBarnebidragResultat,
        omgjøringGrunnlagInput: BeregnGrunnlag,
        omgjøringOrkestratorGrunnlag: OmgjøringOrkestratorGrunnlag,
        skalFatteVedtak: Boolean,
    ): List<ResultatVedtak> {
        try {
            val omgjøringGrunnlagJustert = justerTilPeriodeHvisBarnetBlir18ÅrIBeregningsperioden(omgjøringGrunnlagInput)
            val omgjøringGrunnlag = omgjøringGrunnlagJustert.beregnGrunnlag
            val stønad = omgjøringOrkestratorGrunnlag.stønad
            val omgjørVedtakId = omgjøringOrkestratorGrunnlag.omgjørVedtakId
            val påklagetVedtakId = omgjøringOrkestratorGrunnlag.påklagetVedtakId
            val omgjøringperiode = omgjøringGrunnlag.periode
            val erResultatUtenPerioder = omgjøringResultat.beregnetBarnebidragPeriodeListe.isEmpty()
            secureLogger.debug { "Komplett klageberegning kjøres for stønad $stønad og påklaget vedtak $omgjørVedtakId" }

            var omgjørVedtak = vedtakService.hentVedtak(omgjørVedtakId)
                ?: omgjøringFeilet("Fant ikke omgjort vedtak med id $omgjørVedtakId")

            val påklagetVedtak = if (omgjørVedtakId != påklagetVedtakId) {
                vedtakService.hentVedtak(påklagetVedtakId) ?: omgjøringFeilet("Fant ikke omgjort vedtak med id $omgjørVedtakId")
            } else {
                omgjørVedtak
            }

            // Hvis barn ikke finnes i originale vedtak så betyr det at det var en R-barn som ble lagt til på omgjort vedtak i ettertid.
            // Bruk derfor påklaget vedtak
            if (omgjørVedtak.finnStønadsendring(stønad) == null && omgjørVedtakId != påklagetVedtakId) {
                omgjørVedtak = påklagetVedtak
            }

            if (omgjørVedtak.finnStønadsendring(stønad) == null) {
                // Tilfeller hvor revurderingsbarn er lagt til i klagebehandlingen som ikke var del av opprinnelig vedtak
                // Da skal ikke barnets resultat orkestreres
                // fordi det ikke var del av originale vedtaket og har derfor ikke noe historikk å orkestrere
                val vedtakstype = if (omgjøringOrkestratorGrunnlag.gjelderKlage) Vedtakstype.KLAGE else Vedtakstype.ENDRING
                val beregnetFraDato =
                    omgjøringResultat.beregnetBarnebidragPeriodeListe.minOfOrNull { it.periode.fom }?.atDay(1)
                        ?: omgjøringGrunnlagInput.periode.fom.toLocalDate()
                return listOf(
                    ResultatVedtak(
                        ...
                        beregnet = true,
                        beregnetFraDato = beregnetFraDato,
                    ),
                    ResultatVedtak(
                        resultat = omgjøringResultat,
                        vedtakstype = vedtakstype,
            ...
            val omgjørVedtakVirkningstidspunkt = omgjørVedtak.finnVirkningstidspunktForStønad(stønad)
            ...
            }.gjørOmTilÅpenPeriodeHvisEnesteVedtak(!åpenSluttperiode)
        } catch (e: Exception) {
    ...
    private fun byggVedtak(context: OmgjøringeOrkestratorContext): List<ResultatVedtak> = when {
        ...
        !context.nyVirkningErEtterOpprinneligVirkning && !context.erResultatUtenPerioder && context.skalFatteVedtak ->
        ...
        context.erResultatUtenPerioder || context.nyVirkningErEtterOpprinneligVirkning || !context.skalFatteVedtak ->
    ...
    private fun omgjøringScenarioVirkningFørEllerLikOpprinneligVirkning(context: OmgjøringeOrkestratorContext): List<ResultatVedtak> {
        ...
        val periodeSomSkalOpphøres =
            finnPeriodeSomSkalOpphøres(
                ...
                omgjøringResultat.beregnetBarnebidragPeriodeListe.isEmpty(),
            )
        ...
        val delvedtakListe = buildList {
            ...
            add(
                ResultatVedtak(
                    ...
                    beregnetFraDato = if (omgjøringResultat.beregnetBarnebidragPeriodeListe.isEmpty()) {
                        context.omgjøringsperiode.fom.toLocalDate()
                    } else {
                        omgjøringResultat.beregnetBarnebidragPeriodeListe.minOf {
                            it.periode.fom
                        }.atDay(1)
                    },
                ),
        ...
        val sammenslåttVedtak =
            ResultatVedtak(
                ...
                omgjøringsvedtak = false,
                vedtakstype = context.omgjøringsvedtakVedtaktstype,
                beregnetFraDato = if (resultatSlåttSammen.beregnetBarnebidragPeriodeListe.isEmpty()) {
                    context.omgjøringsperiode.fom.toLocalDate()
                } else {
                    resultatSlåttSammen.beregnetBarnebidragPeriodeListe.minOf {
                        it.periode.fom
                    }.atDay(1)
                },
            )
    ...
    private fun omgjøringScenarioVirkningEtterOpprinneligVirkning(context: OmgjøringeOrkestratorContext): List<ResultatVedtak> {
        ...
        val delvedtakListe = buildList {
            ...
            if (!context.erResultatUtenPerioder) {
                add(
                    ResultatVedtak(
    ...
    private fun opprettDelvedtakFraVedtakslisten(
        ...
        beløpshistorikk.groupBy { it.vedtaksid }.entries.sortedWith(compareBy(nullsLast()) { it.key }).forEach { (vedtaksid, it) ->
            ...
            val resultat = when {
                (vedtaksid == null || it.any { it.revurderAldersjustering }) && it.any { it.aldersjuster } -> {
                    it.filter { it.aldersjuster }.forEach { periode ->
                        val resultat = utførAldersjusteringEllerIndeksreguleringHvisNødvendig(
                            historikk,
                        ...
                        } else if (periode.revurderAldersjustering && komplettVedtak != null) {
                            val resultatFraVedtak = vedtakTilResultatPeriode(komplettVedtak, it)
                            beregnetBarnebidragResultatListe.add(resultatFraVedtak)
                            historikk.add(resultatFraVedtak)
                        }
                    }
                    null
                }
    ...
    private fun List<BeløpshistorikkPeriodeInternal>.finnFørsteIndeksår(
        ...
        }.groupBy { it.vedtaksid }.minByOrNull { (_, perioder) -> perioder.minOf { it.periode.fom } }?.value
            // Hvis ingen historikk så betyr det at det er revurderingsbarn som ikke har noen perioder. Setter til neste år for å unngå indeksreg
            ?: return YearMonth.now().plusYears(1).year
    ...
    private fun List<BeløpshistorikkPeriodeInternal>.fyllPåPerioderForAldersjusteringEllerIndeksregulering(
        ...
        val minYear = minOfNullable(førsteIndeksår, beløshistorikkMedKlage.minOf { it.periode.fom.year })!!
        ...
        val maxYear = when {
            ...
            beregnForPerioderEtterKlage -> {
                // TODO: Legg til sjekk om det er lagt til sjablon for indeks for inneværende år istedenfor
                if (YearMonth.now().month.value < 7) {
        ...
        for (year in minYear..maxYear) {
            ...
            val hasPeriodStartingInJuly = beløshistorikkMedKlage.any {
                it.periode.fom == julyFirst && (!beregnForPerioderEtterKlage || omgjøringsperiode.fom != julyFirst)
    ...
    private fun finnVedtakIderMellomOmgjortVirkningOgNyVirkning(context: OmgjøringeOrkestratorContext): List<BeløpshistorikkPeriodeInternal> {
        ...
        } else if (beløpshistorikkFørOmgjortVedtak.beløpshistorikk.isEmpty()) {
            emptyList()
        } else {
        ...
        return nyBeløpshistorikk.map {
            val erIndeksregulering = if (it.vedtaksid != null && it.periode.fom.month.value == 7) {
                val vedtak = vedtakService.hentVedtak(it.vedtaksid!!)
                vedtak?.type == Vedtakstype.INDEKSREGULERING
            } else {
                false
            }
            // Hvis det er indeksregulering i historikken så skal orkestreringen revurdere om det skal utføres aldersjustering istedenfor indeksregulering
            // Det kan skje hvis aldersjustering ble basert på omgjort vedtak men virkningstidspunkt ble flyttet til etter aldersjustering tidspunkt
            it.copy(
                revurderAldersjustering = erIndeksregulering,
                aldersjuster = erIndeksregulering,
            )
    ...
    private fun finnPeriodeSomSkalOpphøres(
        ...
        ingenBeregningsresultat: Boolean = true,
    ): ÅrMånedsperiode? {
        ...
        return if (tidligstePeriodeFom == null &&
            (
                omgjørVedtakVirkningstidspunkt < omgjøringsperiode.fom ||
                    // Hvis det er revurderingsbarn og resultatet er tom pga at BP har nok evne. Opphør periode som ble revurdert
                    (ingenBeregningsresultat && omgjørVedtakVirkningstidspunkt == omgjøringsperiode.fom)
                )
        ) {
            val periodeFra = YearMonth.of(omgjørVedtakVirkningstidspunkt.year, omgjørVedtakVirkningstidspunkt.monthValue)
            ÅrMånedsperiode(
                fom = periodeFra,
                til = if (periodeFra == omgjøringsperiode.fom) null else omgjøringsperiode.fom,
            )
    ...
    private fun utførAldersjusteringEllerIndeksreguleringHvisNødvendig(
        ...
        val vedtakFraGrunnlag = if (manuellAldersjustering?.grunnlagFraVedtak != null && !manuellAldersjustering.grunnlagFraOmgjøringsvedtak) {
            ...
            if (erKlagevedtak || ((manuellAldersjustering != null) && manuellAldersjustering.grunnlagFraOmgjøringsvedtak)) {
        ...
        val resultatAldersjustering =
            utførAldersjustering(
                ...
                beløpshistorikkFørOmgjortVedtak,
                personobjekter = context.omgjøringsresultat.grunnlagListe.hentAllePersoner() as List<GrunnlagDto>,
    ...
    private fun finnOmgjøringOpphørsdato(omgjøringsvedtak: VedtakDto, stønad: Stønadsid, opphørsdato: YearMonth?): YearMonth? {
        val omgjøringsvedtakSistePeriode = omgjøringsvedtak.finnStønadsendring(stønad)!!.periodeListe.maxByOrNull { it.periode.fom } ?: return null
    ...
    private fun slåSammenVedtak(
        ...
        // Indeks og aldersjustering perioder som overlapper ble brukt for 12% sjekken og skal derfor ikke inkluderes i endelig vedtaket
    ...
    private fun sorterOgJusterPerioder(
        ...
        return sortert.mapIndexed { indeks, resultatPeriode ->
            ...
            val erSistePeriodeLøpendeIkkeOpphør = erSistePeriode && resultatPeriode.periode.til == null && resultatPeriode.resultat.beløp != null
            val nesteFom = if (!erSistePeriode) sortert.getOrNull(indeks + 1)?.periode?.fom else null
            val tilDato = when {
                (erBeregningsperiodeLøpende || erSistePeriodeLøpendeIkkeOpphør) && erSistePeriode && opphørsdato != null -> opphørsdato
    ...
    private fun sjekkMotMinimumsgrenseForEndring(
        ...
        val delberegningEndringSjekkGrensePeriodeResultat =
            delberegningEndringSjekkGrensePeriodeV2(
        ...
        val resultatPeriodeListe = lagResultatPerioderV2(
            sluttberegningPeriodeResultat = omgjøringResultat.grunnlagListe,
    ...
    private fun utførAldersjustering(
        ...
        val søknadsbarn =
            personobjekter.find { it.personIdent == stønad.kravhaver.verdi }?.tilDto()
