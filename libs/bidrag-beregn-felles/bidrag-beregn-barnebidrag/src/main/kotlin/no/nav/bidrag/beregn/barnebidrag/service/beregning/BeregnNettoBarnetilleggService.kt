...
internal object BeregnNettoBarnetilleggService : BeregnService() {
    ...
    fun delberegningNettoBarnetillegg(
        ...
        val nettoBarnetilleggPeriodeGrunnlag = NettoBarnetilleggMapper.mapNettoBarnetilleggGrunnlag(
            mottattGrunnlag = mottattGrunnlag,
            referanseTilRolle = referanseTilRolle,
        )

        // Lager liste over bruddperioder
        val bruddPeriodeListe = lagBruddPeriodeListeNettoBarnetillegg(
        ...
        bruddPeriodeListe.forEach { bruddPeriode ->
            val nettoBarnetilleggBeregningGrunnlag = lagNettoBarnetilleggBeregningGrunnlag(
        ...
        return slåSammenLikeObjekter(
            ...
            grunnlagstype = Grunnlagstype.DELBEREGNING_NETTO_BARNETILLEGG,
            utvidPeriodeTil = ::utvidNettoBarnetilleggPeriodeTil,
        )
    }

    // Spesifikk implementasjon for å utvide periode for DelberegningNettoBarnetillegg. Slår sammen fra-periode fra forrige objekt og til-periode fra
    // nåværende objekt og beholder resten av innholdet fra forrige objekt.
    private fun utvidNettoBarnetilleggPeriodeTil(forrigeGrunnlag: GrunnlagDto, nåværendeGrunnlag: GrunnlagDto): GrunnlagDto {
        val forrigeNettoBarnetillegg = forrigeGrunnlag.tilInnholdMedReferanse<DelberegningNettoBarnetillegg>()
        val nåværendeNettoBarnetillegg = nåværendeGrunnlag.tilInnholdMedReferanse<DelberegningNettoBarnetillegg>()

        val utvidetPeriode = forrigeNettoBarnetillegg.innhold.periode.copy(
            til = nåværendeNettoBarnetillegg.innhold.periode.til,
        )
        val utvidetInnhold = forrigeNettoBarnetillegg.innhold.copy(periode = utvidetPeriode)
        ...
        val barnetilleggBeregningGrunnlagListe = nettoBarnetilleggPeriodeGrunnlag.barnetilleggPeriodeGrunnlagListe
            .filter {
                ...
                BarnetilleggBeregningGrunnlag(
                    ...
                    skattefaktor = it.barnetilleggPeriode.skattefaktor,
                )
            }

        return if (barnetilleggBeregningGrunnlagListe.isEmpty()) {
            null
        } else {
            NettoBarnetilleggBeregningGrunnlag(
