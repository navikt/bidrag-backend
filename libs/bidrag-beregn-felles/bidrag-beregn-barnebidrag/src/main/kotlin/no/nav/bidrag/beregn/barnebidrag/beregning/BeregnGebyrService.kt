...
class BeregnGebyrService(private val sjablonService: SjablonService) {
    ...
    fun beregnGebyr(grunnlagsliste: List<GrunnlagDto>, referanseTilRolle: Grunnlagsreferanse, referansePostfix: String? = null): List<BaseGrunnlag> {
        val sjablonListe = sjablonService.hentSjablontall()
        val gebyrBeregning = grunnlagsliste.beregnGebyrForRolle(referanseTilRolle, referansePostfix)
        ...
        val sluttberegning = GrunnlagDto(
            ...
            referanse = "${Grunnlagstype.SLUTTBEREGNING_GEBYR.name}_$referanseTilRolle${referansePostfix?.let { "_$it" } ?: ""}",
            innhold = POJONode(
                SluttberegningGebyr(
    ...
    private fun List<GrunnlagDto>.beregnGebyrForRolle(
        referanseTilRolle: Grunnlagsreferanse,
        referansePostfix: String?,
    ): DelberegningInntektsbasertGebyrIntern {
        val sjablonListe = sjablonService.hentSjablontall()
        ...
        val grunnlagDelberegning = inntektBeregning.maksInntekt?.let {
            GrunnlagDto(
                referanse = "${Grunnlagstype.DELBEREGNING_INNTEKTSBASERT_GEBYR.name}_$referanseTilRolle${referansePostfix?.let { "_$it" } ?: ""}",
