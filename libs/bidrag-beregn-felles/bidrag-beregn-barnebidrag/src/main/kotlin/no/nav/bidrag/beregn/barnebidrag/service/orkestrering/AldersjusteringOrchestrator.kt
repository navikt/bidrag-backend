...
import no.nav.bidrag.domene.enums.beregning.Resultatkode.Companion.erOpphør
...
class AldersjusteringOrchestrator(
    ...
    fun utførAldersjustering(
        ...
        try {
            ...
            val aldersjusteringPeriodeFom = YearMonth.of(aldersjusteresForÅr, 7)
            ...
            val sisteManuelleVedtak =
                beregnBasertPåVedtak?.let {
                    ...
                    ?: finnSisteManuelleVedtakIgnorerFremtidigOpphør(stønad, aldersjusteringPeriodeFom)

            val sisteManuelleVedtakOpprinnelig = vedtakService.hentOpprinneligVedtakHvisOrkestrertVedtak(sisteManuelleVedtak)

            sisteManuelleVedtakOpprinnelig.validerSkalAldersjusteres(
                stønad,
                aldersjusteresForÅr = aldersjusteresForÅr,
                erManuellJustering = erManuellJustering,
            )

            return sisteManuelleVedtakOpprinnelig.utførOgBeregn(stønad, aldersjusteringPeriodeFom, opphørsdato, beløpshistorikkStønad, personobjekter)
        } catch (e: Exception) {
    ...
    internal fun finnSisteManuelleVedtakIgnorerFremtidigOpphør(
        stønadsid: Stønadsid,
        aldersjusteringPeriodeFom: YearMonth,
        ignorerVedtaksid: List<Int>? = null,
    ): SisteManuelleVedtak {
        val sisteManuelleVedtak =
            vedtakService.finnSisteManuelleVedtak(stønadsid, ignorerVedtaksid)
                ?: aldersjusteresManuelt(SkalAldersjusteresManueltBegrunnelse.FANT_INGEN_MANUELL_VEDTAK)
        val stønadsendring = sisteManuelleVedtak.finnStønadsendring(stønadsid)
        val førstePeriode =
            stønadsendring.periodeListe.minByOrNull { it.periode.fom }
                ?: aldersjusteresManuelt(SkalAldersjusteresManueltBegrunnelse.FANT_INGEN_MANUELL_VEDTAK)

        val vedtaketErOpphør =
            sisteManuelleVedtak.vedtak.type == Vedtakstype.OPPHØR ||
                Resultatkode.fraKode(førstePeriode.resultatkode)?.erOpphør() == true || førstePeriode.beløp == null
        return if (vedtaketErOpphør && førstePeriode.periode.fom > aldersjusteringPeriodeFom) {
            // Hvis det fattet vedtak med fremtidig opphør så skal vi ignorere det og finne siste manuelle vedtak før opphørsvedtaket
            // Bidraget skal aldersjusteres helt til opphørsdatoen. Aldersjusteringen legger på opphørsdato basert på det som ligger i beløpshistorikken slik at opphøret beholdes
            finnSisteManuelleVedtakIgnorerFremtidigOpphør(
                stønadsid,
                aldersjusteringPeriodeFom,
                ignorerVedtaksid = listOf(sisteManuelleVedtak.vedtaksId) + (ignorerVedtaksid ?: emptyList()),
            )
        } else {
            sisteManuelleVedtak
        }
    ...
    internal fun SisteManuelleVedtak.utførOgBeregn(
        ...
        val løpendeStønad = vedtakService
            .hentBeløpshistorikkSistePeriode(stønad, aldersjusteringPeriodeFom.atDay(1).atStartOfDay())
        val sistePeriode = stønadsendring.periodeListe.hentSisteLøpendePeriode(aldersjusteringPeriodeFom)!!
        val sluttberegningSistePeriode = vedtak.grunnlagListe.byggSluttberegningBarnebidragDetaljer(sistePeriode.grunnlagReferanseListe)
        ...
        val resultat = barnebidragApi.beregnAldersjustering(beregningInput)
        val beregnetPeriode = resultat.beregnetBarnebidragPeriodeListe.first()
        val opphørsdato = opphørPåDato ?: løpendeStønad?.periode?.til
        if (opphørsdato != null && beregnetPeriode.periode.fom >= opphørsdato) {
        ...
        val resultatMedGrunnlag = resultat.copy(
            ...
            grunnlagListe = resultat.grunnlagListe + listOf(
                opprettVirkningstidspunktGrunnlag(
                    søknadsbarn.referanse,
                    resultat.beregnetBarnebidragPeriodeListe.first().periode.fom.atDay(1),
                    opphørsdato,
                ),
            ),
        )
        secureLogger.debug {
            "Resultat av beregning av aldersjustering for stønad $stønad og år ${aldersjusteringPeriodeFom.year}: ${resultatMedGrunnlag.beregnetBarnebidragPeriodeListe}"
        }
        return AldersjusteringResultat(
            vedtaksId,
            løpendeStønad?.beløp,
            resultatMedGrunnlag,
    ...
    private fun SisteManuelleVedtak.validerSkalAldersjusteres(
        ...
        val sluttberegning =
            vedtak.grunnlagListe.byggSluttberegningBarnebidragDetaljer(sistePeriode.grunnlagReferanseListe)
        ...
        if (sluttberegning.bidragJustertForDeltBosted && BigDecimal.ZERO.equals(beløpSistePeriode)) {
            aldersjusteresManuelt(
        ...
        if (sluttberegning.begrensetRevurderingUtført && sluttberegning.bidragJustertTilForskuddssats) {
    ...
    private fun String.validerResultatkkode(resultatSistePeriode: String?, vedtaksId: Int?) = when (this) {
        ...
        -> skalIkkeAldersjusteres(
            ...
            if (Resultatkode.fraKode(this)?.erOpphør() == true) {
                skalIkkeAldersjusteres(
                    SkalIkkeAldersjusteresBegrunnelse.BIDRAGET_HAR_OPPHØRT,
                    resultat = resultatSistePeriode,
                    vedtaksid = vedtaksId,
    ...
    private fun SisteManuelleVedtak.byggGrunnlagForBeregning(
        ...
        return BeregnGrunnlagAldersjustering(
            periode = ÅrMånedsperiode(aldersjusteringPeriodeFom, aldersjusteringPeriodeFom.plusMonths(1)),
            ...
            beløpshistorikkListe = listOf(
                beløpshistorikkStønad?.tilGrunnlag(personobjekter, stønad, identUtils)
                    ?: vedtakService.hentBeløpshistorikkTilGrunnlag(
                        stønad,
                        personobjekter,
                        LocalDateTime.now().withYear(aldersjusteringPeriodeFom.year),
                    ),
            ),
        )
    }
    ...
    private fun StønadPeriodeDto?.validerSkalAldersjusteres(sak: BidragssakDto) {
        ...
        if (sak.kategori == Sakskategori.UTLAND) skalIkkeAldersjusteres(SkalIkkeAldersjusteresBegrunnelse.SAKEN_TILHØRER_UTLAND)
    }
