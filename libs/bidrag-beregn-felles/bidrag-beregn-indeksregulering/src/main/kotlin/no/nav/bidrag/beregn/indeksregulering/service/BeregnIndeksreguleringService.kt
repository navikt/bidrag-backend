package no.nav.bidrag.beregn.indeksregulering.service
...
internal class BeregnIndeksreguleringService : BeregnService() {
    ...
    fun beregn(grunnlag: BeregnIndeksreguleringGrunnlag): List<GrunnlagDto> {
        secureLogger.debug {
            "Beregning av indeksregulering barnebidrag - følgende request mottatt: ${commonObjectmapper.writeValueAsString(grunnlag)}"
        }
        ...
        val beløpshistorikk18År = grunnlag.beløpshistorikkListe
            .filtrerOgKonverterBasertPåEgenReferanse<BeløpshistorikkGrunnlag>(
                grunnlagType = Grunnlagstype.BELØPSHISTORIKK_BIDRAG_18_ÅR,
            )
            .map { it ->
                Beløpshistorikk(
                    referanse = it.referanse,
        ...
        val beløpshistorikkOppfostringsbidrag = grunnlag.beløpshistorikkListe
            .filtrerOgKonverterBasertPåEgenReferanse<BeløpshistorikkGrunnlag>(
                grunnlagType = Grunnlagstype.BELØPSHISTORIKK_OPPFOSTRINGSBIDRAG,
            )
            .map { it ->
                Beløpshistorikk(
                    referanse = it.referanse,
        ...
        val gjelderBarnReferanse = grunnlag.personobjektListe.hentPersonMedIdent(grunnlag.stønadsid.kravhaver.verdi)!!.referanse
        ...
        val grunnlagBeregning = lagIndeksreguleringBeregningGrunnlag(
            ...
            beløpshistorikk = beløpshistorikk ?: beløpshistorikk18År ?: beløpshistorikkOppfostringsbidrag
                ?: throw IllegalArgumentException("Beløpshistorikk ikke funnet for stønadstype ${grunnlag.stønadsid.type}"),
        ...
        val resultatGrunnlagListe = mapDelberegningResultatGrunnlag(
            grunnlagReferanseListe = resultatliste.flatMap { it.grunnlagsreferanseListe }.distinct(),
