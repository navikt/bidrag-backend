package no.nav.bidrag.beregn.boforhold.service
...
internal class BoforholdAndreVoksneService {
    fun beregnBoforholdAndreVoksne(
        ...
        secureLogger.debug { "Resultat beregning om BP bor med andre voksne: ${tilJsonString(resultat)}" }
    ...
    private fun beregnPerioder(
        ...
        //        Hvis alle perioder i behandledeBostatusopplysninger har kilde = Offentlig så skal det gjøres en ny beregning basert på
        //        offentlige perioder, likt alternativ 1d.
        ...
        val behandledeOpplysningerErOffentlige = behandledeBostatusopplysninger.isNotEmpty() &&
            behandledeBostatusopplysninger.all { it.kilde == Kilde.OFFENTLIG }
        ...
        val komplettOffentligTidslinje = fyllUtMedBorIkkeMedAndreVoksnePerioder(
            virkningstidspunkt = virkningstidspunkt,
            liste = sammenslåtteOffentligePerioder,
        )

        if (endreBostatus == null) {
            if (behandledeOpplysninger.isNotEmpty() && !behandledeOpplysningerErOffentlige) {
    ...
    private fun fyllUtMedBorIkkeMedAndreVoksnePerioder(virkningstidspunkt: LocalDate, liste: List<Bostatus>): List<Bostatus> {
    ...
    private fun slåSammenPerioderOgJusterPeriodeTom(liste: List<Bostatus>): List<Bostatus> {
        ...
        for (indeks in liste.sortedBy { it.periodeFom }.indices) {
            if (indeks < liste.size - 1) {
                ...
                periodeTom = liste[indeks].periodeTom ?: liste[indeks + 1].periodeFom!!.minusDays(1)
    ...
    private fun behandleEndringer(
        ...
        when (endreBostatus.typeEndring) {
            TypeEndring.SLETTET -> {
                ...
                return if (endredePerioder.isEmpty()) {
                    emptyList()
                } else {
                    // Gjør en sammenslåing av perioder med lik bostatuskode og justerer periodeTom
                    slåSammenPerioderOgJusterPeriodeTom(endredePerioder)
                }
            }
