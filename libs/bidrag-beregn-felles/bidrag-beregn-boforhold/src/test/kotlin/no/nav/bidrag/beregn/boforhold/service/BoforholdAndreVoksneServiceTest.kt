...
import no.nav.bidrag.beregn.boforhold.TestUtil
...
internal class BoforholdAndreVoksneServiceTest {
    ...
    private var beregnTilDato = LocalDate.now().plusMonths(1).withDayOfMonth(1)
...
//            resultat1.size shouldBe 1
    ...
    @Test
    fun `test at periodeTom settes til null`() {
        boforholdAndreVoksneService = BoforholdAndreVoksneService()
        val mottatteBoforhold = BoforholdVoksneRequest(
            innhentedeOffentligeOpplysninger = listOf(
                Husstandsmedlemmer(
                    ...
                    borISammeHusstandListe = listOf(
                        Bostatus(
                            periodeFom = LocalDate.of(2024, 12, 20),
                            periodeTom = null,
                            bostatus = null,
                            kilde = Kilde.OFFENTLIG,
                        ),
        ...
        val virkningstidspunkt = LocalDate.of(2024, 11, 1)
        beregnTilDato = LocalDate.of(2024, 12, 1)
        val resultat = boforholdAndreVoksneService.beregnBoforholdAndreVoksne(virkningstidspunkt, mottatteBoforhold, beregnTilDato = beregnTilDato)

        assertSoftly {
            Assertions.assertNotNull(resultat)
            resultat.size shouldBe 1
            resultat[0].periodeFom shouldBe LocalDate.of(2024, 11, 1)
            resultat[0].periodeTom shouldBe null
            resultat[0].bostatus shouldBe Bostatuskode.BOR_IKKE_MED_ANDRE_VOKSNE
            resultat[0].kilde shouldBe Kilde.OFFENTLIG
        }
    ...
    @Test
    fun `test at offentlige opplysninger beregnes på nytt hvis endreBostatus = null og alle behandledeBostatusopplysninger har kilde = offentlig`() {
        boforholdAndreVoksneService = BoforholdAndreVoksneService()
        val mottatteBoforhold = BoforholdVoksneRequest(
            innhentedeOffentligeOpplysninger = listOf(
                Husstandsmedlemmer(
        ...
        beregnTilDato = LocalDate.of(2026, 4, 1)
        val resultat = boforholdAndreVoksneService.beregnBoforholdAndreVoksne(virkningstidspunkt, mottatteBoforhold, beregnTilDato = beregnTilDato)
