...
internal class SivilstandServiceV2 {
    fun beregn(virkningstidspunkt: LocalDate, sivilstandRequest: SivilstandRequest): List<Sivilstand> {
        ...
        if (sivilstandRequest.endreSivilstand == null) {
            // 1
            val manuelleBehandledeOpplysninger = behandledeOpplysninger.filter { it.kilde == Kilde.MANUELL }
            if (manuelleBehandledeOpplysninger.isNotEmpty()) {
                // 1a + 1b
                // Virkningstidspunkt er endret og/eller offentlige perioder er oppdatert og perioder i manuelleBehandledeOpplysninger skal
                ...
                val sammenslåttListe =
                    slåSammenPrimærOgSekundærperioder(virkningstidspunkt, manuelleBehandledeOpplysninger, offentligePerioder)
    ...
    private fun behandleOffentligePerioder(virkningstidspunkt: LocalDate, sivilstandRequest: SivilstandRequest): List<Sivilstand> {
        ...
        var periodeFomAktivSivilstand: LocalDate?
        if (aktivSivilstand.isNotEmpty()) {
            val sivilstandskode = finnSivilstandskode(aktivSivilstand.first().type!!)
            periodeFomAktivSivilstand = aktivSivilstand.first().gyldigFom
                ?: aktivSivilstand.first().bekreftelsesdato
                ?: aktivSivilstand.first().registrert?.toLocalDate()
                ?: virkningstidspunkt
            periodeFomAktivSivilstand = if (sivilstandskode == Sivilstandskode.BOR_ALENE_MED_BARN) {
                periodeFomAktivSivilstand.withDayOfMonth(1)
            } else {
                periodeFomAktivSivilstand.plusMonths(1)?.withDayOfMonth(1)
            }

            if (periodeFomAktivSivilstand != null && periodeFomAktivSivilstand.isBefore(virkningstidspunkt.plusDays(1))) {
                return listOf(
                    Sivilstand(
        ...
        val sivilstandPDLBoListe = sortertOffentligePeriodeListe.mapIndexed { indeks, sivilstand ->
            ...
            if (sivilstand.historisk == true && periodeFom!!.isAfter(periodeFomAktivSivilstand?.minusDays(1))) {
        ...
        val filtrertSivilstandPDLBoListe =
            sivilstandPDLBoListe.filterNotNull()
    ...
    private fun slåSammenPrimærOgSekundærperioder(
        ...
        // Sjekker offentlige perioder og justerer periodeFom og periodeTom der disse overlapper med primærperioder
