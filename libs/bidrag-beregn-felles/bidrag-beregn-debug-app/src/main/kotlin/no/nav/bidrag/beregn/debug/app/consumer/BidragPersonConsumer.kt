...
@Service
class BidragPersonConsumer(@Value("\${BIDRAG_PERSON_URL}") bidragPersonUrl: URI, @Qualifier("azure") restTemplate: RestTemplate) :
    ...
    @BrukerCacheable(CacheConfig.Companion.PERSON_CACHE)
    fun hentPerson(ident: String): PersonDto = postForNonNullEntity(hentPersonUri, PersonDto(Personident(ident)))

    @BrukerCacheable(CacheConfig.Companion.PERSON_FØDSELSDATO_CACHE)
    @Retryable(
