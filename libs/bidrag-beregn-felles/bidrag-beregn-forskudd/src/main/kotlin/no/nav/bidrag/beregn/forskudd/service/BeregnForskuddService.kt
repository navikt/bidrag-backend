...
import no.nav.bidrag.beregn.forskudd.service.ForskuddCoreMapper.justerTilPeriodeHvisBarnetBlir18ÅrIBeregningsperioden
...
internal class BeregnForskuddService(private val forskuddCore: ForskuddCore = ForskuddCore()) : BeregnService() {
    fun beregn(grunnlag: BeregnGrunnlag): BeregnetForskuddResultat {
        secureLogger.debug { "Forskuddsberegning - følgende request mottatt: ${commonObjectmapper.writeValueAsString(grunnlag)}" }
        ...
        secureLogger.debug { "Forskuddsberegning - grunnlag for beregning: ${commonObjectmapper.writeValueAsString(grunnlagTilCore)}" }
        ...
        secureLogger.debug {
            "Forskuddberegning - resultat av beregning: ${commonObjectmapper.writeValueAsString(resultatFraCore.beregnetForskuddPeriodeListe)}"
        }
        ...
        secureLogger.debug { "Forskuddsberegning - returnerer følgende respons: ${commonObjectmapper.writeValueAsString(respons)}" }
