...
class BPsBeregnedeTotalbidragBeregning : FellesBeregning() {
    ...
    fun beregn(grunnlag: LøpendeBidragGrunnlagCore): ResultatBeregning {
        ...
        grunnlag.løpendeBidragCoreListe.forEach {
            ...
            val reduksjonUnderholdskostnad = if (it.erVedtakKildeBBM) {
                (beregnetBeløpAvrundet - it.faktiskBeløp).coerceAtLeast(BigDecimal.ZERO)
            } else {
                ((it.bruttoBidragEtterBarnetilleggBM ?: BigDecimal.ZERO) - (it.bruttoBidragEtterBarnetilleggBP ?: BigDecimal.ZERO)).coerceAtLeast(
                    BigDecimal.ZERO,
                )
            }
            ...
            beregnetBidragPerBarnListe.add(
                BeregnetBidragPerBarn(
                    ...
                    erVedtakKildeBBM = it.erVedtakKildeBBM,
