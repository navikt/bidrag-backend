...
internal class BeregnInntektService {
    fun beregn(grunnlag: BeregnValgteInntekterGrunnlag): BeregnValgteInntekterResultat {
        secureLogger.debug { "Beregning av inntekt - følgende request mottatt: ${commonObjectmapper.writeValueAsString(grunnlag)}" }
        ...
        secureLogger.debug { "Beregning av inntekt - returnerer følgende respons: ${commonObjectmapper.writeValueAsString(respons)}" }
    ...
    private fun summerOgPeriodiserFellesInntekter(
        ...
        val generelleInntekterListe = grunnlag.grunnlagListe
            ...
            .filter { it.inntektGjelderBarn == null }
            .map {
    ...
    private fun summerOgPeriodiserInntekterPerBarn(
        ...
        grunnlag.barnListe.forEach { barn ->
...
//            }
            ...
            val barnInntekterListe = grunnlag.grunnlagListe
                ...
                .filter {
                    it.inntektGjelderBarn.let { gjelderBarn ->
                        gjelderBarn == null ||
                            (gjelderBarn.ident == barn.ident && barn.stønadstype == gjelderBarn.stønadstype)
                    }
                }
                .map {
                    InntektsgrunnlagPeriode(
