...
abstract class CoreMapper {
    ...
    // unngå at avtalen blir tatt hensyn til.
    fun justerTilPeriodeHvisBarnetBlir18ÅrIBeregningsperioden(mottattGrunnlag: BeregnGrunnlag): BeregnGrunnlagJustert {
        ...
        return if (periodeSøknadsbarnetFyller18År.isBefore(mottattGrunnlag.periode.til)) {
            var justertTilPeriode = periodeSøknadsbarnetFyller18År.plusMonths(1)
            if (justertTilPeriode.isBefore(mottattGrunnlag.periode.fom)) {
                justertTilPeriode = mottattGrunnlag.periode.fom
            }
            BeregnGrunnlagJustert(
                beregnGrunnlag = mottattGrunnlag.copy(
                    periode = mottattGrunnlag.periode.copy(til = justertTilPeriode),
                ),
