...
abstract class BeregnService {
    ...
    // Slår sammen like objekter (som har samme innhold i grunnlagsreferanseListe) for en gitt grunnlagstype (delberegning).
    // Denne funksjonen krever at den kallende klassen implementerer utvidPeriodeTil for den aktuelle typen (funksjonsnavnet her = innholdet i
    // parameter utvidPeriodeTil).
    protected fun slåSammenLikeObjekter(
        grunnlagsliste: List<GrunnlagDto>,
        grunnlagstype: Grunnlagstype,
        utvidPeriodeTil: (GrunnlagDto, GrunnlagDto) -> GrunnlagDto,
    ): List<GrunnlagDto> {
        val (delberegningGrunnlag, andreGrunnlag) = grunnlagsliste
            .partition { it.type == grunnlagstype }

        // Hvis det er 0 eller 1 grunnlag av denne typen, er det ingenting å slå sammen
        if (delberegningGrunnlag.size <= 1) {
            return grunnlagsliste
        }

        val sammenslåtteDelberegningsgrunnlag =
            slåSammenGrunnlagMedLikeReferanser(delberegningGrunnlag = delberegningGrunnlag, utvidPeriodeTil = utvidPeriodeTil)

        // Returnerer de sammenslåtte delberegningsgrunnlagene med de andre grunnlagene
        return andreGrunnlag + sammenslåtteDelberegningsgrunnlag
    }

    protected fun slåSammenGrunnlagMedLikeReferanser(
        delberegningGrunnlag: List<GrunnlagDto>,
        utvidPeriodeTil: (GrunnlagDto, GrunnlagDto) -> GrunnlagDto,
    ): List<GrunnlagDto> = delberegningGrunnlag.fold(emptyList()) { akkumulert, nåværende ->
        val forrige = akkumulert.lastOrNull()

        if (forrige != null && forrige.grunnlagsreferanseListe == nåværende.grunnlagsreferanseListe) {
            // Slå sammen nåværende med forrige ved å utvide perioden
            // utvidPeriodeTil må implementeres som en funksjon i den kallende klassen
            val utvidetGrunnlag = utvidPeriodeTil(forrige, nåværende)
            akkumulert.dropLast(1) + utvidetGrunnlag
        } else {
            // Legg til som nytt grunnlag
            akkumulert + nåværende
        }
    }
    ...
    data class BeregnGrunnlagJustert(
        ...
        val virkningFraPeriode: YearMonth? = null,
    )
}
