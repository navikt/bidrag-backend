...
sealed class Periode<T> : Comparable<Periode<T>> where T : Comparable<T>, T : Temporal {
    ...
    open infix fun snitt(annen: Periode<T>): Periode<T>? = if (!overlapper(annen)) {
        null
    } else if (this == annen) {
        this
    } else {
    ...
    open infix fun union(annen: Periode<T>): Periode<T> = if (overlapper(annen) || this.påfølgesAv(annen) || annen.påfølgesAv(this)) {
        lagPeriode(
            minOf(fom, annen.fom),
    ...
    override fun compareTo(other: Periode<T>): Int = Comparator.comparing(Periode<T>::fom).thenComparing(Periode<T>::tilEllerMax).compare(this, other)
...
fun <T> List<Periode<T>>.erSammenhengende(): Boolean where T : Comparable<T>, T : Temporal = this.sorted().foldIndexed(true) { index, acc, periode ->
    if (index == 0) {
        acc
    } else {
...
fun <T> List<Periode<T>>.harOverlappende(): Boolean where T : Comparable<T>, T : Temporal = this
    .sorted()
    .zipWithNext { a, b ->
        a.overlapper(b)
    }.any { it }
