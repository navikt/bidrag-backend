...
import no.nav.bidrag.dokument.journalpost.configuration.CacheConfig.Companion.TILGANG_PERSON_CACHE
import no.nav.bidrag.dokument.journalpost.configuration.CacheConfig.Companion.TILGANG_SAK_CACHE
import no.nav.bidrag.dokument.journalpost.configuration.CacheConfig.Companion.TILGANG_TEMA_CACHE
import no.nav.bidrag.dokument.journalpost.exception.fantIkkeSak
...
class BidragTilgangskontrollConsumer(
    ...
    fun sjekkTilgangSak(saksnummer: String): Boolean {
        return try {
            postForEntity<TilgangskontrollResponse>(
    ...
    fun sjekkTilgangTema(tema: String): Boolean {
        return try {
            postForEntity<TilgangskontrollResponse>(createUri("/v2/api//tilgang/tema"), TilgangTilTemaRequest(tema))?.harTilgang ?: false
        } catch (e: HttpStatusCodeException) {
            if (e.statusCode == HttpStatus.FORBIDDEN) return false
            throw e
        }
