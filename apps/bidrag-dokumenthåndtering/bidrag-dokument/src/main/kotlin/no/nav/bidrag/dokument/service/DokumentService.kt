...
class DokumentService(
    ...
    fun hentDokumentMetadata(dokumentRef: DokumentRef): List<DokumentMetadata> = if (dokumentRef.erForKilde(Kilde.MIDLERTIDLIG_BREVLAGER)) {
    ...
    @Timed("erFerdigstilt")
    fun erFerdigstilt(dokumentreferanse: String): ResponseEntity<Boolean> = bidragJournalpostConsumer.erFerdigstilt(dokumentreferanse).toResponseEntity()

    protected fun <T : Any> ResponseEntity<T>.toResponseEntity(): ResponseEntity<T> {
        ...
        headers
            ...
            .forEach { (key, value) ->
                value.firstOrNull()?.let {
                    response.header(key, value.firstOrNull()?.toString() ?: "")
                }
            }
    ...
    private fun hentDokumentData(dokument: DokumentRef): ResponseEntity<ByteArray> = if (dokument.erForKilde(Kilde.MIDLERTIDLIG_BREVLAGER)) {
    ...
    private fun hentDokumentRefMedRiktigKilde(dokumentRef: DokumentRef): DokumentRef = if (dokumentRef.erForKilde(Kilde.FORSENDELSE) && dokumentRef.hasDokumentId()) {
    ...
    private fun hentOgMergeAlleDokumenter(
        ...
        val dokumentBytes =
            runBlocking(Dispatchers.IO + SecurityCoroutineContext() + RequestContextAsyncContext()) {
                dokumentList
                    .map {
                        async {
                            hentDokument(
    ...
    private fun mapDokumentTilDokumentRef(dokument: DokumentMetadata): DokumentRef = DokumentRef(
    ...
    private fun parseDokumentString(dokumenterString: List<String>): List<DokumentRef> = dokumenterString
        .stream()
