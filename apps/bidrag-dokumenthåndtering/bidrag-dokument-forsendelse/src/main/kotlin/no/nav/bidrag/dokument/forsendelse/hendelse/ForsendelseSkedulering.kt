...
class ForsendelseSkedulering(
    ...
    @Scheduled(cron = $$"${FERDIGSTILL_NOTAT_FORSENDELSE_SCHEDULE}")
    @SchedulerLock(name = "ferdigstillNotatForsendelseUtenJournalpost", lockAtLeastFor = "10m")
    @Transactional
    fun ferdigstillNotatForsendelseUtenJournalpostSkeduler() {
        ferdigstillNotatForsendelseUtenJournalpost()
    }

    /**
     * Sjekker forsendelser med status UNDER_PRODUKSJON og type NOTAT hvor alle dokumenter er ferdigstilt.
     * Hvis journalpost ikke er opprettet for forsendelsen, ferdigstilles forsendelsen slik at journalpost opprettes og status oppdateres.
     * Dette er en fallback for tilfeller hvor ferdigstilling ikke ble utført automatisk når dokumentene ble ferdigstilt.
     */
    fun ferdigstillNotatForsendelseUtenJournalpost() {
        val forsendelseListe = forsendelseTjeneste.hentUnderProduksjonNotatMedAlleDokumenterFerdigstilt()
        LOGGER.info {
            "Fant ${forsendelseListe.size} forsendelser med status UNDER_PRODUKSJON og type NOTAT hvor alle dokumenter er ferdigstilt. " +
                "Sjekker om journalpost er opprettet for forsendelsene"
        }
        forsendelseListe.forEach {
            try {
                if (it.journalpostIdFagarkiv.isNullOrEmpty()) {
                    LOGGER.info {
                        "Forsendelse ${it.forsendelseId} med type NOTAT mangler journalpost selv om alle dokumenter er ferdigstilt. " +
                            "Ferdigstiller forsendelse"
                    }
                    ferdigstillForsendelseService.ferdigstillForsendelse(it.forsendelseId!!)
                }
            } catch (e: Exception) {
                LOGGER.error(e) { "Det skjedde en feil ved ferdigstilling av notat forsendelse ${it.forsendelseId}" }
            }
        }
    }
