...
//        erGyldigPDFA(ferdigstillDokumentRequest.fysiskDokument, dokumentreferanse)
    ...
    private fun hentDokumentMetadata(dokument: Dokument): List<DokumentMetadata> = if (dokument.erFraAnnenKilde && dokument.arkivsystem != DokumentArkivSystem.FORSENDELSE) {
    ...
    private fun oppdaterDokumentRedigeringMetadata(
        dokument: Dokument,
        redigeringMetadata: String,
    ): DokumentMetadataDo = redigeringMetadata.let { rd ->
        val metadata = dokument.metadata
