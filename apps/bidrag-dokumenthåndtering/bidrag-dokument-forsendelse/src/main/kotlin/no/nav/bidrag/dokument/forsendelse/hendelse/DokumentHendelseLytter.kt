...
class DokumentHendelseLytter(
    ...
    ): Boolean = dokument.dokumentStatus == DokumentStatus.UNDER_PRODUKSJON &&
        (
    ...
    private fun sendJournalposthendelseHvisKlarForDistribusjon(dokumenter: List<Dokument>) {
        hentForsendelser(dokumenter).forEach {
            if (it.kanDistribueres()) {
                journalpostKafkaHendelseProdusent.publiserForsendelse(it)
            }
        }
    }
            ...
            if (forsendelse.metadata?.skalDistribueresAutomatisk() == true && forsendelse.dokumenter.erAlleFerdigstilt) {
                medApplikasjonKontekst {
                    ...
                    try {
                        ...
                        log.error(e) { "Kunne ikke distribuere forsendelse ${forsendelse.forsendelseId}." }
                    }
                }
            }
        }
    ...
    private fun hentForsendelser(dokumenter: List<Dokument>): List<Forsendelse> = dokumenter
        .mapNotNull { it.forsendelse.forsendelseId }
        .distinct()
        .mapNotNull { forsendelseTjeneste.hentForsendelseInitaliserDokumenter(it) }
