...
interface ForsendelseRepository : CrudRepository<Forsendelse, Long> {
    ...
    @Query(
        "select f from forsendelse f where f.status = 'UNDER_PRODUKSJON' and f.forsendelseType = 'NOTAT' and f.journalpostIdFagarkiv is null " +
            "and exists (select 1 from f.dokumenter d where d.slettetTidspunkt is null) " +
            "and not exists (select 1 from f.dokumenter d where d.slettetTidspunkt is null and d.dokumentStatus not in ('FERDIGSTILT', 'KONTROLLERT'))",
    )
    fun hentUnderProduksjonNotatMedAlleDokumenterFerdigstilt(): List<Forsendelse>
