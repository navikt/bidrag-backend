...
class DokumentMetadataDo : MutableMap<String, String> by hashMapOf() {
    ...
    fun hentDokumentDetaljer(): List<DokumentDetaljer>? = get(DOKUMENT_DETALJER_KEY)?.let { objectMapper.readValue(it, object : TypeReference<List<DokumentDetaljer>?>() {}) }
...
class DokumentMetadataDoConverter : ImmutableType<DokumentMetadataDo>(DokumentMetadataDo::class.java) {
    ...
    override fun fromStringValue(sequence: CharSequence?): DokumentMetadataDo = try {
