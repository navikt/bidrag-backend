...
class BidragDokumentConsumer(
    ...
    fun hentJournal(saksnummer: String): List<JournalpostDto> = getForNonNullEntity(
    ...
    @Retryable(value = [Exception::class], maxAttempts = 3, backoff = Backoff(delay = 200, maxDelay = 1000, multiplier = 2.0))
    fun opprettJournalpost(opprettJournalpostRequest: OpprettJournalpostRequest): OpprettJournalpostResponse? = postForEntity(createUri("/journalpost/JOARK"), opprettJournalpostRequest)

    @Retryable(value = [Exception::class], maxAttempts = 3, backoff = Backoff(delay = 200, maxDelay = 1000, multiplier = 2.0))
    fun erFerdigstilt(dokumentreferanse: String): Boolean = getForNonNullEntity(createUri("/dokumentreferanse/$dokumentreferanse/erFerdigstilt"))

    @Retryable(value = [Exception::class], maxAttempts = 3, backoff = Backoff(delay = 200, maxDelay = 1000, multiplier = 2.0))
    fun hentDokumentMetadata(
        journalpostId: String,
        dokumentId: String?,
    ): List<DokumentMetadata> = optionsForEntity(
        UriComponentsBuilder
            .fromUri(url)
