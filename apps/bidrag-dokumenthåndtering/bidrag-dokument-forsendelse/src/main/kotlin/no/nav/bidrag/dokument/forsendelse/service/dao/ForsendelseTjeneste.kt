...
class ForsendelseTjeneste(
    ...
    fun hentForsendelseMedUnikReferanse(unikReferanse: String): Forsendelse? = forsendelseRepository.hentForsendelseMedUnikReferanse(unikReferanse)

    @Transactional(readOnly = true)
    fun hentForsendelseInitaliserDokumenter(forsendelseId: Long): Forsendelse? = forsendelseRepository.medForsendelseId(forsendelseId)?.also { it.dokumenter.size }
    ...
    fun hentUnderProduksjonNotatMedAlleDokumenterFerdigstilt(): List<Forsendelse> = forsendelseRepository.hentUnderProduksjonNotatMedAlleDokumenterFerdigstilt()

    fun hentDistribuerteForsendelserUtenDistribusjonKanal(limit: Int): List<Forsendelse> = forsendelseRepository.hentDistribuerteForsendelseUtenKanal(Pageable.ofSize(limit), LocalDateTime.now().minusHours(2))
    ...
    fun hentForsendelserOpprettetFørDagensDatoIkkeDistribuert(): List<Forsendelse> = forsendelseRepository.hentUnderProduksjonOpprettetFørDagensDato()
