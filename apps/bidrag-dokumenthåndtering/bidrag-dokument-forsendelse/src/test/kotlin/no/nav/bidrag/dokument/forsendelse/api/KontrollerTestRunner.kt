...
abstract class KontrollerTestRunner : CommonTestRunner() {
    ...
    // Delt her (istedenfor i hver enkelt subklasse) slik at Spring gjenbruker samme test-context
    // for alle subklasser av KontrollerTestRunner, se KontrollerTestRunner-testene for detaljer.
    @MockkBean
    ...
    ): ResponseEntity<OpprettForsendelseRespons> = httpHeaderTestRestTemplate.postForEntity<OpprettForsendelseRespons>(
        rootUri(),
    ...
    ): ResponseEntity<OppdaterForsendelseResponse> = httpHeaderTestRestTemplate.exchange(
    ...
    ): ResponseEntity<DokumentRespons> = httpHeaderTestRestTemplate.postForEntity<DokumentRespons>(
    ...
    ): ResponseEntity<List<JournalpostDto>> = httpHeaderTestRestTemplate.exchange(
    ...
    ): ResponseEntity<ForsendelseResponsTo> = httpHeaderTestRestTemplate.getForEntity<ForsendelseResponsTo>(
    ...
    protected fun utførHentJournalpostMedFeil(
        forsendelseId: String,
        saksnummer: String? = null,
    ): ResponseEntity<String> = httpHeaderTestRestTemplate.getForEntity<String>(
    ...
    protected fun utførHentJournalpost(
        forsendelseId: String,
        saksnummer: String? = null,
    ): ResponseEntity<JournalpostResponse> = httpHeaderTestRestTemplate.getForEntity<JournalpostResponse>(
        "${rootUri()}/journal/$forsendelseId${saksnummer?.let { "?saksnummer=$it" }}",
    )

    protected fun utførHentAvvik(forsendelseId: String): ResponseEntity<List<AvvikType>> = httpHeaderTestRestTemplate.exchange(
    ...
    ): ResponseEntity<List<DokumentMetadata>> = httpHeaderTestRestTemplate.exchange<List<DokumentMetadata>>(
    ...
    fun utførHentDokument(
        forsendelseId: String,
        dokumentreferanse: String,
    ): ResponseEntity<ByteArray> = httpHeaderTestRestTemplate.getForEntity<ByteArray>(
