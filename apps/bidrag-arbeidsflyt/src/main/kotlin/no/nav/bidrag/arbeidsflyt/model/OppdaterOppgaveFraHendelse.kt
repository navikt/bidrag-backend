...
class OppdaterOppgaveFraHendelse(
    ...
    private fun overforOppgaveTilJournalforendeEnhet(oppgaveHendelse: OppgaveData) {
        ...
        LOGGER.info {
            "Oppgave ${oppgaveHendelse.id} har oppgavetype=${oppgaveHendelse.oppgavetype} med tema BID men ligger på en ikke journalførende " +
    ...
    private fun endreOppgaveTypeTilJournalforingEllerFerdigstill(oppgaveHendelse: OppgaveData) {
        ...
        if (oppgaver.harJournalforingsoppgaver()) {
            LOGGER.info {
                "Oppgave ${oppgaveHendelse.id} har oppgavetype=${oppgaveHendelse.oppgavetype} med tema BID men tilhørende journalpost har status MOTTATT. " +
    ...
    private fun erJournalforendeEnhet(enhetNr: String?): Boolean {
        val ignoreEnhet = listOf(ENHET_FAGPOST, ENHET_IT_AVDELINGEN, ENHET_YTELSE) + JOURNALFØRENDE_KLAGE_ENHETER
