...
class JsonMapperService {
    fun mapBehandlingHendelse(hendelse: String): BehandlingHendelse = try {
        commonObjectmapper.readValue(hendelse, BehandlingHendelse::class.java)
    } finally {
        secureLogger.debug { "${"Leser hendelse: {}"} $hendelse" }
    }

    fun mapJournalpostHendelse(hendelse: String): JournalpostHendelse = try {
        commonObjectmapper.readValue(hendelse, JournalpostHendelse::class.java)
    } finally {
        secureLogger.debug { "${"Leser hendelse: {}"} $hendelse" }
    }

    fun mapOppgaveHendelseV2(hendelse: String): OppgaveKafkaHendelse = try {
