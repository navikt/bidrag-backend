...
data class OppgaverForHendelse(
    val dataForHendelse: List<OppgaveData>,
) {
    fun erEndringAvTildeltEnhetsnummer(journalpostHendelse: JournalpostHendelse): Boolean = journalpostHendelse.harEnhet() &&
        dataForHendelse
            .stream()
    ...
    fun erEndringAvAktoerId(journalpostHendelse: JournalpostHendelse): Boolean = journalpostHendelse.harAktorId() &&
        dataForHendelse
            .stream()
    ...
    fun harIkkeJournalforingsoppgave(): Boolean = !harJournalforingsoppgaver()

    fun erJournalforingsoppgaverTildeltSaksbehandler(): Boolean = hentJournalforingsOppgaver().any { !it.tilordnetRessurs.isNullOrEmpty() }

    fun harJournalforingsoppgaver() = dataForHendelse.isNotEmpty() &&
        dataForHendelse
            .stream()
            .anyMatch { it.oppgavetype == JOURNALFORINGSOPPGAVE }
    ...
    fun hentBehandleDokumentOppgaverSomSkalOppdateresForNyttDokument(journalpostId: String): List<OppgaveData> {
        ...
        return dataForHendelse
            .filter { it.oppgavetype == OppgaveType.BEH_SAK.name }
    ...
    fun skalOppdatereEllerOppretteBehandleDokumentOppgaver(
        journalpostId: String,
        saker: List<String>,
    ): Boolean = !harBehandleDokumentOppgaveForSaker(journalpostId, saker)
