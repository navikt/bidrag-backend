...
//                        oppgaveType = finnOppgavetypeForStønadstype(førsteBarn.behandlingstema),
    ...
    private fun erAvsluttet(søknadsid: Long?): Boolean = try {
        hentSøknadStatus(søknadsid)?.erAvsluttet == true
    } catch (e: Exception) {
        false
    }

    private fun hentSøknadBehandlerEnhet(søknadsid: Long?) = try {
        if (søknadsid == null) {
            null
        } else {
    ...
    fun finnOppgavetypeForStønadstype(søktOm: Behandlingstema?): OppgaveType = when (søktOm) {
        Behandlingstema.MOTREGNING -> OppgaveType.IN
        Behandlingstema.AVSKRIVNING -> OppgaveType.GEN
        else -> OppgaveType.BEH_SAK
    }

    fun finnFagområdeForSøknad(stønadstype: Stønadstype?): String = when (stønadstype) {
