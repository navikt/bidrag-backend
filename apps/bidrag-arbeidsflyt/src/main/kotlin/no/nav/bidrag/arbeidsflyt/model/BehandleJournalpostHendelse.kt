...
class BehandleJournalpostHendelse(
    ...
    fun hentArbeidsfordeling(): String {
        ...
        if (tema == Fagomrade.FARSKAP) {
            LOGGER.info {
                "Journalposthendelse med journalpostId ${journalpostHendelse.journalpostId} har tema FAR. Bruker enhet $ENHET_FARSKAP ved arbeidsfordeling"
            }
    ...
    private fun validerGyldigDataForBehandleDokument() {
        if (!journalpostHendelse.harTittel) {
            throw ManglerDataForBehandleDokument(
                "Kan ikke opprette/oppdatere behandle dokument oppgave fordi hendelse mangler tittel",
            )
        }
        if (!journalpostHendelse.harDokumentDato) {
            throw ManglerDataForBehandleDokument(
                "Kan ikke opprette/oppdatere behandle dokument oppgave fordi hendelse mangler dokument dato",
            )
        }
        if (!journalpostHendelse.harSporingsdataEnhet) {
            throw ManglerDataForBehandleDokument(
                "Kan ikke opprette/oppdatere behandle dokument oppgave fordi hendelse mangler enhetsnummer",
            )
        }
    }
