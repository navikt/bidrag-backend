...
import no.nav.bidrag.arbeidsflyt.utils.journalpostId3
import no.nav.bidrag.arbeidsflyt.utils.journalpostResponse
import no.nav.bidrag.arbeidsflyt.utils.oppgaveId1
import no.nav.bidrag.arbeidsflyt.utils.oppgaveId3
import no.nav.bidrag.arbeidsflyt.utils.oppgavetypeJfr
...
class OppgaveHendelseTest : AbstractBehandleHendelseTest() {
    ...
    fun `skal lagre oppgave`() {
        ...
        assertThat(opprettetOppgaveOptional).hasValueSatisfying { oppgave ->
            ...
            assertThat(oppgave.oppgavetype).isEqualTo(oppgavetypeJfr)
    ...
    fun `skal lagre journalforingsoppgave ved endring hvis ikke finnes`() {
        val oppgaveData = createOppgaveData(oppgaveId3, journalpostId = journalpostId3, status = OppgaveStatus.OPPRETTET)
        stubHentOppgave(oppgaveData.id, oppgaveData)

        behandleOppgaveHendelseService.behandleOppgaveHendelse(oppgaveData.toHendelse())

        val endretOppgaveOptional = testDataGenerator.hentOppgave(oppgaveId3)
        assertThat(endretOppgaveOptional.isPresent).isTrue

        assertThat(endretOppgaveOptional).hasValueSatisfying { oppgave ->
            assertThat(oppgave.oppgaveId).isEqualTo(oppgaveId3)
            assertThat(oppgave.journalpostId).isEqualTo(journalpostId3)
            assertThat(oppgave.oppgavetype).isEqualTo(oppgavetypeJfr)
            assertThat(oppgave.status).isEqualTo(OppgaveStatus.OPPRETTET.name)
        }
    ...
    fun `skal endre oppgave`() {
        testDataGenerator.opprettOppgave(createOppgave(oppgaveId3, journalpostId = "UKJENT"))
        val oppgaveData = createOppgaveData(oppgaveId3, journalpostId = journalpostId3, status = OppgaveStatus.OPPRETTET)
        stubHentOppgave(oppgaveData.id, oppgaveData)
    ...
    fun `skal slette oppgave fra databasen nar oppgave lukket`() {
        stubHentJournalpost()
        val oppgaveData =
            createOppgaveData(oppgaveId3, journalpostId = journalpostId3, statuskategori = Oppgavestatuskategori.AVSLUTTET)
        stubHentOppgave(oppgaveData.id, oppgaveData)
        behandleOppgaveHendelseService.behandleOppgaveHendelse(
    ...
    fun `skal opprette oppgave med BID prefix nar det ikke finnes noen aapne jfr oppgaver men journalpost status er mottatt`() {
        ...
        stubHentJournalpost(journalpostResponse(bidJournalpostId1, journalStatus = JournalpostStatus.MOTTATT))
        val oppgaveData =
            createOppgaveData(
                oppgaveId1,
    ...
    fun `skal opprette oppgave nar det ikke finnes noen aapne jfr oppgaver men journalpost status er mottatt`() {
        ...
        stubHentGeografiskEnhet()
        stubHentJournalpost(journalpostResponse(journalpostId1, journalStatus = JournalpostStatus.MOTTATT))
        val oppgaveData =
            createOppgaveData(
                oppgaveId1,
                journalpostId = journalpostId1,
    ...
    fun `skal ikke opprette oppgave nar det ikke finnes noen aapne jfr oppgaver men journalpost status er mottatt men ikke har tema bidrag`() {
        ...
        stubHentJournalpost(journalpostResponse(journalpostId1, journalStatus = JournalpostStatus.MOTTATT, tema = "BAR"))
        val oppgaveData =
            createOppgaveData(
                oppgaveId1,
                journalpostId = journalpostId1,
    ...
    fun `skal opprette oppgave nar oppgave endret fra JFR til BEH_SAK og journalpost er mottatt`() {
        ...
        stubHentGeografiskEnhet()
        stubHentJournalpost(journalpostResponse(journalpostId1, journalStatus = JournalpostStatus.MOTTATT))
        testDataGenerator.opprettOppgave(createOppgave(oppgaveId1))
    ...
    fun `skal ikke opprette oppgave nar oppgave endret fra JFR til BEH_SAK og journalpost ikke mottatt`() {
        stubHentOppgaveSok(emptyList())
        testDataGenerator.opprettOppgave(createOppgave(oppgaveId1))
        stubHentJournalpost(journalpostResponse(bidJournalpostId1, journalStatus = JournalpostStatus.JOURNALFØRT))
        val oppgaveData = createOppgaveData(oppgaveId3, journalpostId = journalpostId1, aktoerId = aktørId, oppgavetype = "BEH_SAK")
        stubHentOppgave(oppgaveData.id, oppgaveData)
