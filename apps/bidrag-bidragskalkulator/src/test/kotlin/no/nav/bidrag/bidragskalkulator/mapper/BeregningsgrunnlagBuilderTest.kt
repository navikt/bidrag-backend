...
class BeregningsgrunnlagBuilderTest {
    ...
    inner class MottattFaktiskUtgiftgrunnlagTest {
        ...
        fun `skal bygge mottatt faktisk utgift med riktig grunnlagstype`() {
            val barn = lagBarnDto(fødselsdato = LocalDate.now().minusYears(1), samværklasse = Samværsklasse.SAMVÆRSKLASSE_3, barnetilsyn = BarnetilsynDto(BigDecimal("1200")))

            val resultat = builder.byggMottattFaktiskUtgift(
                barn.fødselsdato,
                "Person_Søknadsbarn_0",
                barn.barnetilsyn?.månedligUtgift ?: BigDecimal.ZERO,
            )
    ...
    ): BeregningKontekst = BeregningKontekst(
