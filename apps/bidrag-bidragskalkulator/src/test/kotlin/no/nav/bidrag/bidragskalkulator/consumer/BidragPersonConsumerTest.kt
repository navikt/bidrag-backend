...
class BidragPersonConsumerTest {
    ...
    fun `skal kaste NoContentException når 404 fra bidrag-person`() {
        val ident = genererFødselsnummer()
        val exception = HttpServerErrorException.create(HttpStatus.NOT_FOUND, "Not Found", HttpHeaders(), ByteArray(0), StandardCharsets.UTF_8)
    ...
    fun `skal kaste original feil ved 500`() {
        val ident = genererFødselsnummer()
        val exception = HttpServerErrorException.create(HttpStatus.INTERNAL_SERVER_ERROR, "Serverfeil", HttpHeaders(), ByteArray(0), StandardCharsets.UTF_8)
