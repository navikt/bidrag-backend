...
class GlobalExceptionHandler {
    ...
    private fun problem(status: HttpStatus, detail: String, vararg errors: String): ProblemDetail = ProblemDetail.forStatusAndDetail(status, detail).apply {
    ...
    fun handleIllegalArgument(e: IllegalArgumentException): ProblemDetail = problem(HttpStatus.BAD_REQUEST, e.message ?: "Ugyldig forespørsel")
    ...
    fun handleNoContentException(@Suppress("UNUSED_PARAMETER") ex: NoContentException): ResponseEntity<Void> = ResponseEntity.noContent().build()
    ...
    fun handleGeneric(e: Exception, req: HttpServletRequest): ProblemDetail = problem(HttpStatus.INTERNAL_SERVER_ERROR, "Uventet feil oppstod.")

    @ExceptionHandler(GrunnlagNotFoundException::class)
    fun handleGrunnlagNotFound(e: GrunnlagNotFoundException) = ResponseEntity.status(404).body(mapOf("message" to e.message))

    @ExceptionHandler(InntektTransformException::class)
    fun handleInntektTransform(e: InntektTransformException) = ResponseEntity.status(502).body(mapOf("message" to e.message))
}
