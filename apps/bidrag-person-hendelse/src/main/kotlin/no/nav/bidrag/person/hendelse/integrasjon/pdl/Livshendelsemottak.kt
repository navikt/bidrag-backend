...
class Livshendelsemottak(
    ...
    private fun henteAdressebeskyttelse(adressebeskyttelse: Adressebeskyttelse?): Livshendelse.Gradering = if (adressebeskyttelse == null) {
    ...
    ): LocalDate? = if (bostedsadresse == null && kontaktadresse == null && oppholdsadresse == null) {
        null
    } else {
    ...
    private fun erKontaktadresseEndret(kontaktadresse: Kontaktadresse?): Boolean = kontaktadresse != null &&
        (
    ...
    private fun erOppholdsadresseEndret(oppholdsadresse: Oppholdsadresse?): Boolean = oppholdsadresse != null &&
        (
    ...
    ): Folkeregisteridentifikator? = if (folkeregisteridentifikator == null) {
        null
    } else {
    ...
    private fun henteFødselsdato(foedselsdato: no.nav.person.pdl.leesah.foedselsdato.Foedselsdato?): Foedselsdato? = if (foedselsdato == null) {
        null
    } else {
        Foedselsdato(foedselsdato.foedselsdato, foedselsdato.foedselsaar)
    }

    private fun henteInnflytting(innflytting: no.nav.person.pdl.leesah.innflytting.InnflyttingTilNorge?): Innflytting? = if (innflytting == null) {
        null
    } else {
        Innflytting(innflytting.fraflyttingsland?.toString(), innflytting.fraflyttingsstedIUtlandet?.toString())
    }

    private fun henteNavn(navn: no.nav.person.pdl.leesah.navn.Navn?): Navn? = if (navn == null) {
        null
    } else {
        val originaltNavn =
            OriginaltNavn(
    ...
    private fun henteUtflytting(utflytting: no.nav.person.pdl.leesah.utflytting.UtflyttingFraNorge?): Utflytting? = if (utflytting == null) {
        null
    } else {
        Utflytting(
    ...
    private fun henteSivilstand(sivilstand: no.nav.person.pdl.leesah.sivilstand.Sivilstand?): Sivilstand? = if (sivilstand == null) {
        null
    } else {
        Sivilstand(sivilstand.type?.toString(), sivilstand.bekreftelsesdato, sivilstand.gyldigFraOgMed)
    }

    private fun henteVerge(verge: no.nav.person.pdl.leesah.verge.VergemaalEllerFremtidsfullmakt?): VergeEllerFremtidsfullmakt? = if (verge == null) {
        null
    } else {
