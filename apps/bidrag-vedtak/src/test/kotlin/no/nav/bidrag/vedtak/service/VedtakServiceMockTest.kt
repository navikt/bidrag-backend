...
import no.nav.bidrag.vedtak.TestUtil.Companion.KRAVHAVER_IDENT
import no.nav.bidrag.vedtak.TestUtil.Companion.MOTTAKER_IDENT
import no.nav.bidrag.vedtak.TestUtil.Companion.SKYLDNER_IDENT
        ...
        Mockito.`when`(persistenceServiceMock.hentAlleEngangsbeløpForVedtak(MockitoHelper.any(Int::class.java))).thenReturn(
            listOf(
                byggEngangsbeløp(
                    ...
                    skyldner = Personident(SKYLDNER_IDENT).toString(),
                    kravhaver = Personident(KRAVHAVER_IDENT).toString(),
                    mottaker = Personident(MOTTAKER_IDENT).toString(),
