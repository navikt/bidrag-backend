...
class VedtakControllerTest {
    ...
    fun `skal hente alle data for et vedtak`() {
        ...
        // Henter forekomster
        val response = securedTestRestTemplate.getForEntity<VedtakDto>("${makeFullContextPath()}/vedtak/$opprettetVedtakId")
    ...
    fun `skal opprette nytt vedtak og hente det via behandlingsreferanse`() {
        ...
        val response = securedTestRestTemplate.getForEntity<List<Int>>("${makeFullContextPath()}/vedtak/hent-vedtak-for-behandlingsreferanse/$kilde/$behandlingsreferanse")
    ...
    private fun fullUrlForNyttVedtak(): String = UriComponentsBuilder.fromUriString(makeFullContextPath() + VedtakController.OPPRETT_VEDTAK).toUriString()

    private fun fullUrlHentVedtakForUnikReferanse(): String = UriComponentsBuilder.fromUriString(makeFullContextPath() + VedtakController.HENT_VEDTAK_FOR_UNIK_REFERANSE).toUriString()

    private fun fullUrlForOppdaterVedtak(): String = UriComponentsBuilder.fromUriString(makeFullContextPath() + VedtakController.OPPDATER_VEDTAK).toUriString()

    private fun fullUrlForHentVedtak(): String = UriComponentsBuilder.fromUriString(makeFullContextPath() + VedtakController.HENT_VEDTAK).toUriString()

    private fun fullUrlForHentVedtakForBehandlingsreferanse(): String = UriComponentsBuilder.fromUriString(makeFullContextPath() + VedtakController.HENT_VEDTAK_FOR_BEHANDLINGSREFERANSE).toUriString()
    ...
    private fun lesFilOgByggRequest(filnavn: String): HttpEntity<String> {
        ...
        json =
            json
                .replace("\"SKYLDNER\"", "\"${genererFødselsnummer()}\"")
                .replace("\"KRAVHAVER1\"", "\"${genererFødselsnummer()}\"")
                .replace("\"KRAVHAVER2\"", "\"${genererFødselsnummer()}\"")
                .replace("\"KRAVHAVER\"", "\"${genererFødselsnummer()}\"")
                .replace("\"MOTTAKER\"", "\"${genererFødselsnummer()}\"")
    ...
    private fun <T : Any> initHttpEntity(body: T): HttpEntity<T> {
        ...
        return HttpEntity(body, httpHeaders) as HttpEntity<T>
    }
}
