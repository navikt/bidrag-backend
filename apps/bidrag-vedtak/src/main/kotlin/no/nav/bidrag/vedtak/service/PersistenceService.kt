...
class PersistenceService(
    ...
    @Timed
    fun hentVedtak(id: Int): Vedtak = vedtakRepository.findById(id).orElseThrow { IllegalArgumentException(String.format("Fant ikke vedtak med id %d i databasen", id)) }
    ...
    fun hentAllePerioderForStønadsendringListe(stønadsendringsidListe: List<Int>): Map<Int, List<Periode>> {
        if (stønadsendringsidListe.isEmpty()) return emptyMap()
        return periodeRepository.hentAllePerioderForStønadsendringListe(stønadsendringsidListe)
            .groupBy { it.stønadsendring.id }
    }

    fun hentAlleGrunnlagForStønadsendring(stønadsendringId: Int): List<StønadsendringGrunnlag> = stønadsendringGrunnlagRepository.hentAlleGrunnlagForStønadsendring(stønadsendringId)

    fun hentAlleGrunnlagForStønadsendringListe(stønadsendringsidListe: List<Int>): Map<Int, List<StønadsendringGrunnlag>> {
        if (stønadsendringsidListe.isEmpty()) return emptyMap()
        return stønadsendringGrunnlagRepository.hentAlleGrunnlagForStønadsendringListe(stønadsendringsidListe)
            .groupBy { it.stønadsendring.id }
    }
    ...
    fun hentAlleGrunnlagForEngangsbeløp(engangsbeløpId: Int): List<EngangsbeløpGrunnlag> = engangsbeløpGrunnlagRepository.hentAlleGrunnlagForEngangsbeløp(engangsbeløpId)
    ...
    fun hentAlleBehandlingsreferanserForVedtak(id: Int): List<Behandlingsreferanse> = behandlingsreferanseRepository.hentAlleBehandlingsreferanserForVedtak(id)

    fun hentAlleBehandlingsreferanserForVedtakListe(vedtaksidListe: List<Int>): Map<Int, List<Behandlingsreferanse>> {
        if (vedtaksidListe.isEmpty()) return emptyMap()
        return behandlingsreferanseRepository.hentAlleBehandlingsreferanserForVedtakListe(vedtaksidListe)
            .groupBy { it.vedtak.id }
    }
    ...
    fun slettAlleStønadsendringGrunnlagForStønadsendring(stønadsendringsid: Int): Int = stønadsendringGrunnlagRepository.slettStønadsendringGrunnlagForStønadsendring(stønadsendringsid)
    ...
    fun hentManuelleVedtakForBp(skyldnerListe: List<String>): List<Stønadsendring> = stønadsendringRepository.hentVedtakForBpMedIdenthistorikk(
        medInnkreving = Innkrevingstype.MED_INNKREVING.toString(),
        endring = Beslutningstype.ENDRING.toString(),
        manuelt = Vedtakskilde.MANUELT.toString(),
        indeksregulering = Vedtakstype.INDEKSREGULERING.toString(),
        aldersjustering = Vedtakstype.ALDERSJUSTERING.toString(),
        skyldnerListe = skyldnerListe,
    )
}
