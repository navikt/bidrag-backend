...
interface StønadsendringGrunnlagRepository : CrudRepository<StønadsendringGrunnlag, StønadsendringGrunnlagPK> {
    ...
    @Query(
        "select sg from StønadsendringGrunnlag sg join fetch sg.grunnlag where sg.stønadsendring.id in :stønadsendringsidListe order by sg.stønadsendring.id, sg.grunnlag.id",
    )
    fun hentAlleGrunnlagForStønadsendringListe(stønadsendringsidListe: List<Int>): List<StønadsendringGrunnlag>
