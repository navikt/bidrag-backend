...
class Databehandler(
            ...
            try {
                val nyForespørsel =
                    if (originalForespørsel.alleBarnHarFylt15år) {
                        databasetjeneste.oppdaterForespørselTilÅIkkeKreveSamtykke(
                ...
                brukernotifikasjonkonsument.varsleOmAutomatiskInnsending(nyForespørsel.hovedpartIdent, nyForespørsel.motpartIdent)
            } catch (e: Exception) {
                log.error(
                    e,
    ...
    private fun deaktivereJournalførteForespørsler() {
        val journalførteAktiveForespørsler = databasetjeneste.henteIdTilAktiveForespørsler(
            LocalDateTime.now().minusDays(FORESPØRSLER_SYNLIGE_I_ANTALL_DAGER_ETTER_SISTE_STATUSOPPDATERING),
            true,
        )
    ...
    private fun ferdiglogg(varselSendt: Int, antallVurderteForespørsler: Int) {
        ...
        val loggstreng = if (alleForeldreBleVarslet) {
