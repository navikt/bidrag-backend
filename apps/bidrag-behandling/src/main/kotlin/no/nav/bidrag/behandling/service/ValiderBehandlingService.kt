...
class ValiderBehandlingService(
    ...
    private fun kanBidragV1BehandlesINyLøsning(request: KanBehandlesINyLøsningRequest): String? {
        ...
        if (request.erRevurdering() && request.søknadsid != null &&
            UnleashFeatures.BEHANDLE_BARNEBIDRAG_FLERE_BARN_LØPENDE_BIDRAG.isEnabled
        ) {
            val sammeknytninger = hentSammenknytningerHovedsøknad(request.søknadsid)
            if (sammeknytninger != null && sammeknytninger.søknader.isNotEmpty()) {
                @Suppress("ktlint:standard:max-line-length")
                return "Kan ikke behandle FF i gammel løsning. Feilregistrer revurderingssøknaden først og utløs FF i ny løsning."
            }
        }
