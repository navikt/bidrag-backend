...
fun BidragssakDto.hentRolleMedFnr(fnr: String): RolleDto = roller.firstOrNull { hentNyesteIdent(it.fødselsnummer?.verdi) == hentNyesteIdent(fnr) }
    ?: fantIkkeRolleISak(saksnummer.verdi, fnr)

val BidragssakDto.barn get() = roller.filter { it.type == Rolletype.BARN }

fun List<BidragssakDto>.filtrerSakerHvorPersonErBP(bpIdent: String) = filter {
