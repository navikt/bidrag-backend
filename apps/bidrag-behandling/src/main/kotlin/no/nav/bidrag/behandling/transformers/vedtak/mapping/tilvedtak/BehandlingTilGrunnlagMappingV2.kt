...
class BehandlingTilGrunnlagMappingV2(
    ...
    fun Behandling.tilPersonobjekter(
        ...
        val bidragsmottakere =
            if (bidragsmottaker != null) {
                listOf(bidragsmottaker.tilGrunnlagPerson())
            } else {
                alleBidragsmottakere.map { it.tilGrunnlagPerson() }
            }
        return (
            bidragsmottakere +
                listOf(
    ...
    fun Behandling.byggInnhentetGrunnlag(
        ...
        val sortertGrunnlagsListe =
            grunnlag
                .hentSisteAktiv()
                .filter { grunnlag ->
                    grunnlag.gjelderBarnRolle == null || byggForRoller.any { it.erSammeRolle(grunnlag.gjelderBarnRolle!!) }
                }.filter { grunnlag ->
                    val byggForBidragsmottaker = byggForRoller.find { it.rolletype == Rolletype.BIDRAGSMOTTAKER }
                    when (grunnlag.rolle.rolletype) {
                        Rolletype.BIDRAGSPLIKTIG -> {
                            true
                        }

                        Rolletype.BIDRAGSMOTTAKER -> {
                            byggForBidragsmottaker?.erSammeRolle(grunnlag.rolle) ?: true
                        }

                        else -> {
                            byggForRoller.any { it.erSammeRolle(grunnlag.rolle) }
                        }
                    }
                }
        ...
        val innhentetHusstandsmedlemmer =
            sortertGrunnlagsListeIkkeBearbeidet.tilInnhentetHusstandsmedlemmer(personobjekter, this, byggForRoller)
