...
fun innhentetGrunnlagHarFlereRelatertePersonMedSammeId(): Nothing = throw HttpClientErrorException(
    HttpStatus.BAD_REQUEST,
    "Innhentet grunnlag for husstandsmedlemmer har flere relaterte personer med samme personId",
)

fun inntektManglerSøknadsbarn(inntektsrapportering: Inntektsrapportering): Nothing = grunnlagByggingFeilet(
...
): Nothing = grunnlagByggingFeilet(
    "Mangler rolle $rolletype" +
        "${behandlingId?.let { " i behandling $it" } ?: ""}${fødselsnummer?.let { " med fødselsnummer $it" } ?: ""}",
)

fun grunnlagByggingFeilet(message: String): Nothing = throw HttpClientErrorException(
    HttpStatus.BAD_REQUEST,
