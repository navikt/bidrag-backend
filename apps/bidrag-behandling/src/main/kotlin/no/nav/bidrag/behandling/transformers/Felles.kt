...
fun Behandling.finnSistePeriodeLøpendePeriodeInnenforEldsteVirkning(rolle: Rolle): StønadPeriodeDto? {
    val eksisterendeVedtak = hentGrunnlagBeløpshistorikkForRolle(rolle, false)
    val stønad = eksisterendeVedtak.konverterTilStønadDto() ?: return null

    val sistePeriode = stønad.periodeListe.maxByOrNull { it.periode.fom } ?: return null
    return if (løperPeriodeEtterEldsteVirkningstidspunkt(sistePeriode.periode)) {
        sistePeriode
    } else {
        null
    }
...
fun Rolle.harLøpendeBidragFørOpphørEllerLøpende(): Boolean {
    val løperBidragEtterEldsteVirkning = (
        behandling.finnesLøpendeBidragForRolleEldsteVirkning(this) ||
            behandling.privatAvtale
                .filter {
                    val sistePeriodeTom = it.perioder.maxByOrNull { it.fom }?.tom
                    sistePeriodeTom == null || sistePeriodeTom > behandling.eldsteVirkningstidspunkt
                }.any {
