...
data class ResultatadBeregningOrkestrering(
    ...
    val enesteSak get() = saker.firstOrNull()
...
class BehandlingTilVedtakMapping(
    ...
    fun Behandling.byggOpprettVedtakRequestBidragAlle(request: FatteVedtakRequestDto? = null): VedtakRequstDto = if (vedtakstype == Vedtakstype.ALDERSJUSTERING) {
        VedtakRequstDto(
    ...
    fun hentBeregningBarnebidrag(
        ...
        val saker =
            behandling.saker.map {
                sakConsumer.hentSak(it)
            }
        val beregning =
    ...
    ): OpprettVedtakRequestDto = mapper.run {
    ...
    fun Behandling.byggOpprettVedtakRequestSplittetFFOmgjøringEtterFFBleTrukket(
        ...
        return groupedBarnSøknader.mapNotNull { (søknadsid, barnForSøknad) ->
            ...
            val saksnummer = søknadsbarnFiltered.first().saksnummer
            val erRevurderingsbarn = beregningerForSøknad.all { it.barn.erRevurderingsbarn }
            val bidragsmottaker = behandling.bidragsmottakerForSak(saksnummer) ?: behandling.bidragsmottaker
            val byggGrunnlagForSøknadsbarn =
                if (erRevurderingsbarn) {
                    søknadsbarnFiltered
                } else {
                    søknadsbarn.filter { !it.erRevurderingsbarn && it.saksnummer == saksnummer }
                }

            val byggGrunnlagForRoller = byggGrunnlagForSøknadsbarn + listOfNotNull(bidragsmottaker)
            ...
            val beregninger =
                beregningerForSøknad.map { beregningBarn ->
                    ...
                    byggVedtakForKlage(
                        ...
                        bleFFTrukket = true,
                        byggForRoller = byggGrunnlagForRoller,
                    )
                }
            ...
            .map { resultatVedtak ->
                ...
                val søknadsbarn =
                    behandling.søknadsbarn.find {
                        it.erSammeRolle(
                            beregningBarn.barn.ident!!.verdi,
                            beregningBarn.barn.stønadstype,
                        )
                    }
                val innkreving =
                    if (klagevedtakErEnesteVedtak) {
                        søknadsbarn?.innkrevingstype ?: behandling.innkrevingstype!!
                    } else {
                        Innkrevingstype.UTEN_INNKREVING
                    }
    ...
    fun opprettVedtakRequestDelvedtakV2(
        ...
        klagevedtakErEnesteVedtak: Boolean,
    ): List<ResultatDelvedtak> = beregningBarn
        .flatMap {
            opprettVedtakRequestDelvedtak(behandling, saker, request, it, klagevedtakErEnesteVedtak)
    ...
    fun byggVedtakForKlage(
        ...
        byggForRoller: List<Rolle> = behandling.søknadsbarn,
    ): OpprettVedtakRequestDto {
        mapper.run {
            val byggForSøknadsbarn = byggForRoller.filter { it.rolletype == Rolletype.BARN }
            ...
            return behandling.byggOpprettVedtakRequestObjekt(request?.enhet, byggForSøknadsbarn).copy(
                ...
                stønadsendringPerioder.map { it ->
                    ...
                    OpprettStønadsendringRequestDto(
                        ...
                        behandling.roller
                            .reelMottakerEllerBidragsmottaker(
                                saker.hentRolleMedFnr(it.barn.ident!!),
                            ),
    ...
    private fun Behandling.byggOpprettVedtakRequestSplittetFF(
        ...
        return groupedBarnSøknader.mapNotNull {
            ...
            val bidragsmottaker = behandling.bidragsmottakerForSak(førsteSøknadsbarn.saksnummer) ?: behandling.bidragsmottaker
            val byggGrunnlagForSøknadsbarn =
                if (erRevurderingsbarn) {
                    søknadsbarnISøknad
                } else {
                    søknadsbarn.filter { !it.erRevurderingsbarn && it.saksnummer == førsteSøknadsbarn.saksnummer }
                }
            ...
            // TODO: Hvordan filtrere ut grunnlag som gjelder saken?
            @Suppress("ktlint:standard:comment-wrapping")
            val grunnlagslisteForSak =
                beregning.grunnlagsliste /*beregning.grunnlagsliste.filter {
                    if (it.type != Grunnlagstype.PERSON_BARN_BIDRAGSMOTTAKER) return@filter true
                    val gjelder = beregning.grunnlagsliste.hentPersonMedReferanse(it.gjelderReferanse) ?: return@filter true
                    val gjelderBM = gjelder.type == Grunnlagstype.PERSON_BIDRAGSMOTTAKER
                    !gjelderBM || bidragsmottaker!!.ident == gjelder.personIdent
                }*/
            ...
            mapper.run {
                ...
                val grunnlagListeVedtak =
                    byggGrunnlagForVedtak(
                        grunnlagslisteForSak.hentAllePersoner().toMutableSet() as MutableSet<GrunnlagDto>,
                        byggForRoller = byggGrunnlagForRoller,
                    )
                ...
                val grunnlagslisteForVedtak =
                    if (erAvvistRevurdering) {
                        val grunnlagPersoner =
                            grunnlagslisteForSak.hentAllePersoner().filter { p ->
        ...
        mapper.run {
            ...
            return byggOpprettVedtakRequestObjekt(request?.enhet).copy(
                ...
                stønadsendringPerioder.map { periode ->
                    ...
                    val innkrevingstype =
                        if (erAvvisning || (periode.barn.erRevurderingsbarn && innkrevesDelerAvPeriodene)) {
    ...
    private fun Behandling.mapEngangsbeløpDirekteOppgjør(behandlingSaker: Map<String, BidragssakDto>) = søknadsbarn
        .filter {
    ...
    fun Behandling.byggOpprettVedtakRequestAvslagForBidrag(request: FatteVedtakRequestDto? = null): OpprettVedtakRequestDto = mapper.run {
        val sak = sakConsumer.hentSak(saksnummer)
    ...
    fun Behandling.byggOpprettVedtakRequestAvslagForForskudd(enhet: String? = null): OpprettVedtakRequestDto = mapper.run {
        val sak = sakConsumer.hentSak(saksnummer)
