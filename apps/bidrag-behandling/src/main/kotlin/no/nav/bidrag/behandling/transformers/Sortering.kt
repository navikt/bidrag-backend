...
fun Set<Rolle>.sorterForInntektsbildet() = sortedWith(
    compareBy<Rolle> {
        when (it.rolletype) {
...
fun List<TotalBeregningUtgifterDto>.sorterBeregnetUtgifter() = sortedWith(
...
fun SummerteInntekter<SummertÅrsinntekt>.filtrerUtHistoriskeInntekter() = copy(
    inntekter =
...
fun Collection<Inntekt>.filtrerUtHistoriskeInntekter() = this.filter { inntekt ->
    inntekt.taMed || !inntekt.erHistorisk(this)
}

fun List<Inntekt>.ekskluderYtelserFørVirkningstidspunkt(eksluderYtelserFørVirkningstidspunkt: Boolean = true) = filter {
...
fun List<AndreVoksneIHusstandenDetaljerDto>.sorter() = sortedWith(
...
fun Husstandsmedlem.erSøknadsbarn() = this.behandling.søknadsbarn
    .map { it.ident }
    .contains(this.ident)

fun List<HusstandsmedlemDtoV2>.sortert() = sortedWith(
...
fun Set<Husstandsmedlem>.sortert() = sortedWith(
...
fun List<Inntekt>.sorterEtterDatoOgBarn() = sortedWith(
    compareBy({
