...
class GrunnlagMappingTest {
    ...
    inner class GrunnlagByggerTest {
        @Test
        fun `skal bygge grunnlag for stønad for forskudd`(): Unit = mapper.run {
        ...
        @Test
        fun `skal bygge grunnlag for engangsbeløp for særbidrag`(): Unit = mapper.run {
        ...
        @Test
        fun `skal bygge grunnlag for vedtak`(): Unit = mapper.run {
            val behandling = opprettGyldigBehandlingForBeregningOgVedtak(true)
            behandling.grunnlag =
        ...
        @Test
        fun `skal bygge grunnlag for beregning`(): Unit = mapper.run {
            val behandling = opprettGyldigBehandlingForBeregningOgVedtak(true)
            behandling.inntekter =
                mutableSetOf(
                    Inntekt(
    ...
    inner class SærbidragGrunnlagByggerTest {
        @Test
        fun `skal bygge særbidragskategori grunnlag`(): Unit = mapper.run {
            val behandling = opprettGyldigBehandlingForBeregningOgVedtak(true, typeBehandling = TypeBehandling.SÆRBIDRAG)
            behandling.stonadstype = null
            behandling.engangsbeloptype = Engangsbeløptype.SÆRBIDRAG
        ...
        @Test
        fun `skal bygge utgift direkte betalt`(): Unit = mapper.run {
            val behandling = opprettGyldigBehandlingForBeregningOgVedtak(true, typeBehandling = TypeBehandling.SÆRBIDRAG)
            behandling.stonadstype = null
            behandling.engangsbeloptype = Engangsbeløptype.SÆRBIDRAG
        ...
        @Test
        fun `skal bygge utgiftsposter grunnlag`(): Unit = mapper.run {
            val behandling = opprettGyldigBehandlingForBeregningOgVedtak(true, typeBehandling = TypeBehandling.SÆRBIDRAG)
            behandling.stonadstype = null
            behandling.engangsbeloptype = Engangsbeløptype.SÆRBIDRAG
    ...
    inner class PersonGrunnlagTest {
        @Test
        fun `skal mappe bare ene søknadsbarnet og parter til liste over persongrunnlag`(): Unit = behandlingTilGrunnlagMapping.run {
        ...
        @Test
        fun `skal mappe alle roller til liste over persongrunnlag`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteBehandling(1)
            behandling.roller =
                mutableSetOf(
                    opprettRolle(behandling, testdataBM, 1),
        ...
        @Test
        fun `skal mappe rolle til grunnlag`(): Unit = behandlingTilGrunnlagMapping.run {
        ...
        @Test
        fun `skal mappe husstandsmedlem til grunnlag`(): Unit = behandlingTilGrunnlagMapping.run {
        ...
        @Test
        fun `skal mappe relatertperson til grunnlag`(): Unit = behandlingTilGrunnlagMapping.run {
            assertSoftly(
        ...
        @Test
        fun `skal mappe husstandsmedlem uten ident til grunnlag`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
            assertSoftly(
                Husstandsmedlem(
                    behandling = behandling,
        ...
        @Test
        fun `skal mappe relatertperson med nyeste ident`(): Unit = behandlingTilGrunnlagMapping.run {
        ...
        @Test
        fun `skal mappe husstandsmedlem med nyeste ident`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
            stubHentPersonNyIdent("12345678901", "1231232131")
            assertSoftly(
                Husstandsmedlem(
        ...
        @Test
        fun `skal mappe rolle med nyeste ident`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
    ...
    inner class InntektGrunnlagTest {
        ...
        @Test
        fun `skal ikke mappe inntekt til grunnlag hvis inntekt ikke tilhører søknadsbarn`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
        ...
        @Test
        fun `skal feile mapping av inntektperiode hvis søknadsbarn mangler`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
            behandling.grunnlag =
                opprettInntekterBearbeidetGrunnlag(
                    behandling,
        ...
        @Test
        fun `skal mappe inntekt til grunnlag hvis inneholder inntektliste for Barn og BP`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
            behandling.grunnlag =
                (
        ...
        @Test
        fun `skal legge til grunnlagsliste for innhentet inntekter`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
        ...
        @Test
        fun `skal feile hvis offentlig inntekt mangler grunnlagsreferanseliste`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
        ...
        fun List<GrunnlagDto>.hentInntekt(inntektsrapportering: Inntektsrapportering) = filter {
    ...
    inner class BosstatusGrunnlagTest {
        ...
        fun opprettPerioder(husstandsmedlem: Husstandsmedlem) = mutableSetOf(
            Bostatusperiode(
        ...
        @Test
        fun `skal opprette grunnlag for husstandsmedlem`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteBehandling(1)
        ...
        @Test
        fun `skal opprette grunnlag for husstandsmedlem uten ident`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteBehandling(1)
        ...
        @Test
        fun `skal mappe husstandsmedlem til bosstatus`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
    ...
    inner class SivilstandGrunnlagTest {
        fun opprettSivilstand(behandling: Behandling): MutableSet<Sivilstand> = mutableSetOf(
            Sivilstand(
                behandling = behandling,
                datoFom = LocalDate.parse("2022-01-01"),
                datoTom = LocalDate.parse("2022-06-30"),
        ...
        @Test
        fun `skal opprette grunnlag for sivilstand`(): Unit = behandlingTilGrunnlagMapping.run {
            val behandling = oppretteTestbehandling()
    ...
    inner class BehandlingGrunnlagTest {
        ...
        @Test
        fun `skal opprette grunnlag for søknad`(): Unit = mapper.run {
        ...
        @Test
        fun `skal opprette grunnlag for virkningstidspunkt`(): Unit = mapper.run {
            val behandling = opprettBehandling()
        ...
        @Test
        fun `skal opprette grunnlag for behandlingsreferanser`(): Unit = mapper.run {
        ...
        @Test
        fun `skal opprette grunnlag for behandlingsreferanser med klageid`(): Unit = mapper.run {
        ...
        @Test
        fun `skal opprette grunnlag for notat og ikke ta med notat hvis tomt eller null`(): Unit = mapper.run {
        ...
        @Test
        fun `skal opprette grunnlag for notat`(): Unit = mapper.run {
            val behandling = oppretteTestbehandling(true, setteDatabaseider = true)
            behandling.leggTilNotat("asdasd", NotatGrunnlag.NotatType.VIRKNINGSTIDSPUNKT)
            behandling.leggTilNotat("asdasd", NotatGrunnlag.NotatType.INNTEKT, behandling.bidragsmottaker)
            behandling.leggTilNotat("asdasd", NotatGrunnlag.NotatType.BOFORHOLD, behandling.bidragsmottaker)
