...
class StønadsendringBidragsmottakerTest {
    private val barnIRolleDtoIdent = genererFødselsnummer()
    private val bidragspliktigIdent = genererFødselsnummer()

    @BeforeEach
    fun setup() {
        mockkStatic("no.nav.bidrag.behandling.service.OppslagServiceKt")
    }
    ...
    fun `skal returnere bidragsmottaker når reell mottaker ikke finnes`() {
        val bidragsmottakerIdent = genererFødselsnummer()
